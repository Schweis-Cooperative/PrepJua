import type { VocabularyWord } from '../types/vocabulary';

export const vocabularyData: VocabularyWord[] = [
  {
    id: 'v-new-hw-1',
    word: 'reach out',
    translation: 'Ulaşmak',
    example: 'If you have any questions, don\'t hesitate to reach out to me. - Eğer herhangi bir sorunuz olursa, bana ulaşmaktan çekinmeyin.', 
    category: 'phrase',
  },
  {
    id: 'v-new-hw-2',
    word: 'criticise',
    translation: 'Eleştirmek',
    example: 'It is unfair to criticise him without knowing the full story. - Tüm hikayeyi bilmeden onu eleştirmek haksızlıktır.', 
    category: 'verb',
  },
  {
    id: 'v-new-hw-4',
    word: 'memorise',
    translation: 'Ezberlemek',
    example: 'I need to memorise these new vocabulary words before the test. - Sınavdan önce bu yeni kelimeleri ezberlemem gerekiyor.', 
    category: 'verb',
  },

  {
    id: 'v-u1-extra-01',
    word: 'common',
    translation: 'Ortak',
    example: 'Traffic congestion is a common problem in big cities. - Trafik sıkışıklığı büyük şehirlerde ortak bir sorundur.', // To be filled if needed or left blank for fast track
    category: 'adjective',
  },
  {
    id: 'v-u1-extra-02',
    word: 'exist',
    translation: 'Var olmak, var olan',
    example: 'Many people wonder if life could exist on other planets. - Çoğu insan başka gezegenlerde yaşamın var olup olamayacağını merak ediyor.', // To be filled if needed or left blank for fast track
    category: 'verb',
  },
  {
    id: 'v-u1-extra-03',
    word: 'develop',
    translation: 'Geliştirmek',
    example: 'Reading regularly can help you develop your language skills. - Düzenli okumak dil becerilerinizi geliştirmenize yardımcı olabilir.', // To be filled if needed or left blank for fast track
    category: 'verb',
  },
  {
    id: 'v-u1-extra-04',
    word: 'fluent',
    translation: 'Akıcı (dil konuşma)',
    example: 'She lived in London for five years, so she is completely fluent. - Londra\'da beş yıl yaşadı, bu yüzden tamamen akıcı konuşuyor.', // To be filled if needed or left blank for fast track
    category: 'adjective',
  },
  {
    id: 'v-u1-extra-05',
    word: 'agree',
    translation: 'Katılmak (bir fikre)',
    example: 'I completely agree with your opinion on this matter. - Bu konudaki fikrine tamamen katılıyorum.', // To be filled if needed or left blank for fast track
    category: 'verb',
  },
  {
    id: 'v-u1-extra-06',
    word: 'neither',
    translation: 'İkisi de değil (olumsuz katılım)',
    example: 'I don\'t like coffee, and neither does my brother. - Ben kahve sevmem, kardeşim de sevmez.', // To be filled if needed or left blank for fast track
    category: 'adverb',
  },
  {
    id: 'v-u2-extra-01',
    word: 'press',
    translation: 'Tuşlamak, basmak',
    example: 'Please press the red button to start the machine. - Makineyi başlatmak için lütfen kırmızı tuşa basın.', // To be filled if needed or left blank for fast track
    category: 'verb',
  },
  {
    id: 'v-u2-extra-02',
    word: 'undo',
    translation: 'Geri almak (işlemi)',
    example: 'Is there a way to undo this action on the computer? - Bilgisayarda bu işlemi geri almanın bir yolu var mı?', // To be filled if needed or left blank for fast track
    category: 'verb',
  },
  {
    id: 'v-u2-extra-03',
    word: 'charge',
    translation: 'Şarj etmek',
    example: 'I forgot to charge my phone, so the battery is dead. - Telefonumu şarj etmeyi unuttum, bu yüzden şarjı bitti.', // To be filled if needed or left blank for fast track
    category: 'verb',
  },
  {
    id: 'v-u2-extra-04',
    word: 'business contract',
    translation: 'İş sözleşmesi',
    example: 'They signed a new business contract with an international firm. - Uluslararası bir firmayla yeni bir iş sözleşmesi imzaladılar.', // To be filled if needed or left blank for fast track
    category: 'phrase',
  },
  {
    id: 'v-u2-extra-05',
    word: 'make contact',
    translation: 'Temas kurmak, iletişime geçmek',
    example: 'We are trying to make contact with the manager to discuss the issue. - Sorunu görüşmek üzere yöneticiyle temas kurmaya çalışıyoruz.', // To be filled if needed or left blank for fast track
    category: 'phrase',
  },
  {
    id: 'v-u2-extra-06',
    word: 'emphasis',
    translation: 'Vurgu',
    example: 'The teacher put a strong emphasis on speaking practice. - Öğretmen konuşma pratiğine güçlü bir vurgu yaptı.', // To be filled if needed or left blank for fast track
    category: 'noun',
  },
  // ═══════════════════════════════════════════════════════════════
  // UNIT 3 — Relationships & Multi-word Verbs
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u3-01',
    word: 'have a lot in common',
    translation: 'Ortak çok yönü olmak',
    example: 'My sister and I have a lot in common, but we don\'t like the same music.',
    category: 'phrase',
  },
  {
    id: 'v-u3-02',
    word: 'get on well with',
    translation: 'İyi geçinmek, iyi anlaşmak',
    example: 'I get on well with most of my family, but I don\'t like my aunt very much.',
    category: 'phrase',
  },
  {
    id: 'v-u3-03',
    word: 'have a good relationship with',
    translation: 'İyi bir ilişkisi olmak',
    example: 'I have a good relationship with my cousin. We\'re more like friends.',
    category: 'phrase',
  },
  {
    id: 'v-u3-04',
    word: 'get to know',
    translation: 'Tanımaya başlamak, yakından tanımak',
    example: 'I think you really get to know people well when you go on holiday with them.',
    category: 'phrase',
  },
  {
    id: 'v-u3-05',
    word: 'come from the same background',
    translation: 'Aynı çevreden gelmek',
    example: 'Most of my friends come from the same background as I do.',
    category: 'phrase',
  },
  {
    id: 'v-u3-06',
    word: 'keep in touch',
    translation: 'İletişimi sürdürmek, bağlantıda kalmak',
    example: 'I\'m not very good at keeping in touch with friends when they move far away.',
    category: 'phrase',
  },
  {
    id: 'v-u3-07',
    word: 'friendship',
    translation: 'Arkadaşlık',
    example: 'My longest friendship is with someone I met when I was eight years old.',
    category: 'noun',
  },
  // Multi-word verbs from 3B
  {
    id: 'v-u3-08',
    word: 'grow up',
    translation: 'Büyümek (çocukluktan yetişkinliğe)',
    example: 'Bianca is the sister I grew up with.',
    category: 'verb',
  },
  {
    id: 'v-u3-09',
    word: 'take after',
    translation: 'Benzemek (aile büyüklerine)',
    example: 'I mainly take after my dad — we\'re both good at drawing.',
    category: 'verb',
  },
  {
    id: 'v-u3-10',
    word: 'grow apart',
    translation: 'Giderek birbirinden uzaklaşmak',
    example: 'I don\'t think we\'ll ever grow apart.',
    category: 'verb',
  },
  {
    id: 'v-u3-11',
    word: 'get together',
    translation: 'Bir araya gelmek, buluşmak',
    example: 'We usually get together once or twice a week.',
    category: 'verb',
  },
  {
    id: 'v-u3-12',
    word: 'hang out',
    translation: 'Vakit geçirmek (arkadaşlarla)',
    example: 'Now that we\'re older, we hang out together all the time.',
    category: 'verb',
  },
  {
    id: 'v-u3-13',
    word: 'mix up',
    translation: 'Karıştırmak, birbirine karıştırmak',
    example: 'People used to mix us up all the time — even our parents!',
    category: 'verb',
  },
  {
    id: 'v-u3-14',
    word: 'bring up',
    translation: 'Yetiştirmek, büyütmek (çocuk)',
    example: 'Our parents understood this from the beginning and brought us up to be different people.',
    category: 'verb',
  },
  {
    id: 'v-u3-15',
    word: 'cut off',
    translation: 'Kesmek, kırpmak',
    example: 'We\'d both cut all our hair off.',
    category: 'verb',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 3 — Describing Time
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u3-16',
    word: 'meanwhile',
    translation: 'Bu arada, bu sırada',
    example: 'Meanwhile, Mary stayed in Australia because it was much safer.',
    category: 'adverb',
  },
  {
    id: 'v-u3-17',
    word: 'over',
    translation: 'Boyunca, süresince (bir dönem)',
    example: 'I found their house over the summer when I was on holiday.',
    category: 'preposition',
  },
  {
    id: 'v-u3-18',
    word: 'during',
    translation: 'Sırasında, boyunca',
    example: 'During their stay in Melbourne, they decided they really liked the city.',
    category: 'preposition',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 4 — Change (Wordpower)
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u4-01',
    word: 'adapt to',
    translation: 'Uyum sağlamak, adapte olmak',
    example: 'It takes time to adapt to a new culture.',
    category: 'verb',
  },
  {
    id: 'v-u4-02',
    word: 'adjust',
    translation: 'Ayarlamak, uyum sağlamak',
    example: 'You need to adjust your expectations when travelling.',
    category: 'verb',
  },
  {
    id: 'v-u4-03',
    word: 'transform',
    translation: 'Dönüştürmek, tamamen değiştirmek',
    example: 'Technology has transformed the way we communicate.',
    category: 'verb',
  },
  {
    id: 'v-u4-04',
    word: 'alter',
    translation: 'Değiştirmek (kısmen)',
    example: 'We need to alter the plan slightly.',
    category: 'verb',
  },
  {
    id: 'v-u4-05',
    word: 'convert',
    translation: 'Dönüştürmek, çevirmek',
    example: 'They converted the old factory into apartments.',
    category: 'verb',
  },
  {
    id: 'v-u4-06',
    word: 'exchange',
    translation: 'Takas etmek, değiş tokuş etmek',
    example: 'Can I exchange this shirt for a larger size?',
    category: 'verb',
  },
  {
    id: 'v-u4-07',
    word: 'replace',
    translation: 'Yerine koymak, değiştirmek',
    example: 'We need to replace the old windows.',
    category: 'verb',
  },
  {
    id: 'v-u4-08',
    word: 'swap',
    translation: 'Takas etmek (konuşma dili)',
    example: 'Shall we swap seats?',
    category: 'verb',
  },
  {
    id: 'v-u4-09',
    word: 'vary',
    translation: 'Farklılık göstermek, değişmek',
    example: 'Prices vary depending on the season.',
    category: 'verb',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 5 — Environment & Nature
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u5-01',
    word: 'wildlife',
    translation: 'Yaban hayatı',
    example: 'The wildlife in the national park is amazing — from butterflies to elephants.',
    category: 'noun',
  },
  {
    id: 'v-u5-02',
    word: 'endangered species',
    translation: 'Nesli tehlike altında olan türler',
    example: 'The blue whale and the mountain gorilla are both endangered species.',
    category: 'noun',
  },
  {
    id: 'v-u5-03',
    word: 'recycle',
    translation: 'Geri dönüştürmek',
    example: 'Most countries recycle paper, glass and cardboard.',
    category: 'verb',
  },
  {
    id: 'v-u5-04',
    word: 'climate change',
    translation: 'İklim değişikliği',
    example: 'The climate is changing; some places are getting hotter and some are getting colder.',
    category: 'noun',
  },
  {
    id: 'v-u5-05',
    word: 'damage',
    translation: 'Zarar vermek, hasar',
    example: 'Building new roads damages the environment.',
    category: 'verb',
  },
  {
    id: 'v-u5-06',
    word: 'cut down',
    translation: 'Kesmek (ağaçları), azaltmak',
    example: 'They want to cut down the trees so they can use the land for farming.',
    category: 'verb',
  },
  {
    id: 'v-u5-07',
    word: 'rainforest',
    translation: 'Yağmur ormanı',
    example: 'A rainforest is a tropical, wet place with a lot of trees.',
    category: 'noun',
  },
  {
    id: 'v-u5-08',
    word: 'stream',
    translation: 'Dere, çay',
    example: 'The children jumped across the stream.',
    category: 'noun',
  },
  {
    id: 'v-u5-09',
    word: 'valley',
    translation: 'Vadi',
    example: 'A valley is a low area of land between hills or mountains.',
    category: 'noun',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 5 — Problem (Wordpower)
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u5-10',
    word: 'be aware of',
    translation: 'Farkında olmak',
    example: 'His shoelaces are untied, but he isn\'t aware of the problem.',
    category: 'phrase',
  },
  {
    id: 'v-u5-11',
    word: 'fix',
    translation: 'Tamir etmek, düzeltmek',
    example: 'Juan\'s car won\'t start, but he\'s fixing the problem.',
    category: 'verb',
  },
  {
    id: 'v-u5-12',
    word: 'face',
    translation: 'Yüzleşmek, karşı karşıya kalmak',
    example: 'They\'re facing a lot of problems. Their bills are huge this month.',
    category: 'verb',
  },
  {
    id: 'v-u5-13',
    word: 'cause',
    translation: 'Neden olmak, sebep olmak',
    example: 'Bad weather has caused a lot of problems for traffic.',
    category: 'verb',
  },
  {
    id: 'v-u5-14',
    word: 'solve',
    translation: 'Çözmek',
    example: 'Lena is upset, but her parents are trying to solve the problem.',
    category: 'verb',
  },
  {
    id: 'v-u5-15',
    word: 'tackle',
    translation: 'Ele almak, üstesinden gelmek',
    example: 'The fire quickly got out of control, but they\'re tackling the problem.',
    category: 'verb',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 5 — Giving reasons, results and examples
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u5-16',
    word: 'as a result',
    translation: 'Sonuç olarak',
    example: 'She studied hard; as a result, she passed the exam.',
    category: 'phrase',
  },
  {
    id: 'v-u5-17',
    word: 'because of',
    translation: 'Yüzünden, sebebiyle',
    example: 'It takes years at university because of all the things you have to study.',
    category: 'phrase',
  },
  {
    id: 'v-u5-18',
    word: 'due to',
    translation: 'Nedeniyle, dolayı',
    example: 'Due to bad weather, the flight was delayed.',
    category: 'phrase',
  },
  {
    id: 'v-u5-19',
    word: 'for instance',
    translation: 'Örneğin',
    example: 'There are many ways to help, for instance, volunteering at a shelter.',
    category: 'phrase',
  },
  {
    id: 'v-u5-20',
    word: 'such as',
    translation: 'Gibi, örneğin',
    example: 'I enjoy outdoor activities such as hiking and cycling.',
    category: 'phrase',
  },
  {
    id: 'v-u5-21',
    word: 'since',
    translation: '-den beri / çünkü, madem ki',
    example: 'Since I like finding the best university, I need to do a lot of research.',
    category: 'conjunction',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 6 — Compound Nouns & Travel
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u6-01',
    word: 'boarding pass',
    translation: 'Biniş kartı',
    example: 'You need a boarding pass to get on a plane.',
    category: 'noun',
  },
  {
    id: 'v-u6-02',
    word: 'campsite',
    translation: 'Kamp alanı',
    example: 'A campsite is a place you go to stay in a tent.',
    category: 'noun',
  },
  {
    id: 'v-u6-03',
    word: 'air conditioning',
    translation: 'Klima',
    example: 'Air conditioning is a system for keeping your room cool.',
    category: 'noun',
  },
  {
    id: 'v-u6-04',
    word: 'sightseeing tour',
    translation: 'Gezilecek yerleri görme turu',
    example: 'A sightseeing tour is a guided visit to the famous places in a town or city.',
    category: 'noun',
  },
  {
    id: 'v-u6-05',
    word: 'pedestrian crossing',
    translation: 'Yaya geçidi',
    example: 'It\'s safer to use a pedestrian crossing when you want to cross the street.',
    category: 'noun',
  },
  {
    id: 'v-u6-06',
    word: 'rush hour',
    translation: 'Yoğun saat, trafik saati',
    example: 'Let\'s go at ten o\'clock, when rush hour is over.',
    category: 'noun',
  },
  {
    id: 'v-u6-07',
    word: 'public transport',
    translation: 'Toplu taşıma',
    example: 'Should we drive or use public transport?',
    category: 'noun',
  },
  {
    id: 'v-u6-08',
    word: 'traffic jam',
    translation: 'Trafik sıkışıklığı',
    example: 'Sorry I\'m late. I got stuck in a traffic jam.',
    category: 'noun',
  },
  {
    id: 'v-u6-09',
    word: 'traffic lights',
    translation: 'Trafik ışıkları',
    example: 'Wait for the traffic lights to change from red to green.',
    category: 'noun',
  },
  {
    id: 'v-u6-10',
    word: 'vending machine',
    translation: 'Otomatik satış makinesi',
    example: 'I\'d like a cold drink. Is there a vending machine near here?',
    category: 'noun',
  },
  {
    id: 'v-u6-11',
    word: 'cycle lane',
    translation: 'Bisiklet yolu',
    example: 'That car shouldn\'t be in the cycle lane!',
    category: 'noun',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 6 — Describing Food
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u6-13',
    word: 'creamy',
    translation: 'Kremalı, kremsi',
    example: 'Creamy Mexican avocado and tomato dip with crunchy tortilla chips.',
    category: 'adjective',
  },
  {
    id: 'v-u6-14',
    word: 'spicy',
    translation: 'Baharatlı, acılı',
    example: 'White fish cooked in a spicy Thai sauce with hot green chillies.',
    category: 'adjective',
  },
  {
    id: 'v-u6-15',
    word: 'crunchy',
    translation: 'Çıtır, gevrek',
    example: 'The tortilla chips were nice and crunchy.',
    category: 'adjective',
  },
  {
    id: 'v-u6-16',
    word: 'bitter',
    translation: 'Acı (tat)',
    example: 'A slice of rich Austrian chocolate cake with a bitter orange filling.',
    category: 'adjective',
  },
  {
    id: 'v-u6-17',
    word: 'fresh',
    translation: 'Taze',
    example: 'Tasty Moroccan meatballs cooked in tomato sauce, served with couscous and fresh herbs.',
    category: 'adjective',
  },
  {
    id: 'v-u6-18',
    word: 'light',
    translation: 'Hafif (yemek)',
    example: 'The meal was light and fresh — delicious!',
    category: 'adjective',
  },
  {
    id: 'v-u6-19',
    word: 'raw',
    translation: 'Çiğ, pişmemiş',
    example: 'This hasn\'t been cooked properly. Look! The vegetables are still raw.',
    category: 'adjective',
  },
  {
    id: 'v-u6-20',
    word: 'sour',
    translation: 'Ekşi',
    example: 'This juice is really sour. There\'s too much lemon in it.',
    category: 'adjective',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 6 — Wordpower: go
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u6-22',
    word: 'go (disappear)',
    translation: 'Kaybolmak, bitmek',
    example: 'Where\'s the milk? It\'s gone.',
    category: 'verb',
  },
  {
    id: 'v-u6-23',
    word: 'go well',
    translation: 'İyi gitmek',
    example: 'I had a job interview yesterday. It went really well. I got the job!',
    category: 'phrase',
  },
  {
    id: 'v-u6-24',
    word: 'go wrong',
    translation: 'Ters gitmek, bozulmak',
    example: 'Don\'t worry if it went wrong. Just start again.',
    category: 'phrase',
  },
  {
    id: 'v-u6-25',
    word: 'go with',
    translation: 'Uymak, yakışmak (birlikte)',
    example: 'Do these shoes go with this dress?',
    category: 'phrase',
  },
  {
    id: 'v-u6-26',
    word: 'go bad',
    translation: 'Bozulmak (yiyecek)',
    example: 'Milk that is not refrigerated will eventually go bad.',
    category: 'phrase',
  },
  {
    id: 'v-u6-27',
    word: 'go around',
    translation: 'Dolaşmak, etrafında gitmek',
    example: 'The road goes around the lake. It\'s a nice drive.',
    category: 'phrase',
  },
  {
    id: 'v-u6-28',
    word: 'go off',
    translation: 'Bozulmak / Patlamak / Kapanmak',
    example: 'Yes, but when I got there it had all gone. It went off. I threw it away.',
    category: 'phrase',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 6 — Culture & Advice
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u6-29',
    word: 'culture shock',
    translation: 'Kültür şoku',
    example: 'What do you think the phrase "culture shock" means? What difficulties might it cause?',
    category: 'noun',
  },
  {
    id: 'v-u6-30',
    word: 'insight',
    translation: 'İçgörü, kavrayış',
    example: 'The app provides insights into the culture of the country you\'re visiting.',
    category: 'noun',
  },
  {
    id: 'v-u6-31',
    word: 'integral',
    translation: 'Temel, ayrılmaz, bütünleyici',
    example: 'This became an integral part of the app\'s content.',
    category: 'adjective',
  },
  {
    id: 'v-u6-32',
    word: 'approach',
    translation: 'Yaklaşım',
    example: 'The couple decided not to take the conventional approach to culture.',
    category: 'noun',
  },
  {
    id: 'v-u6-33',
    word: 'at the heart of',
    translation: 'Kalbinde, özünde, merkezinde',
    example: 'They decided to put this at the heart of what CultureMee does.',
    category: 'phrase',
  },
  {
    id: 'v-u6-34',
    word: 'supplement',
    translation: 'Desteklemek, ek olarak eklemek',
    example: 'Videos produced by local people supplement the core content of the app.',
    category: 'verb',
  },
  {
    id: 'v-u6-35',
    word: 'with authority',
    translation: 'Yetkin bir şekilde, otorite ile',
    example: 'People who have visited the country and who can talk with authority about it.',
    category: 'phrase',
  },
  {
    id: 'v-u6-36',
    word: 'augment',
    translation: 'Artırmak, büyütmek, geliştirmek',
    example: 'They hope this community will continue to augment it with their own stories.',
    category: 'verb',
  },

  // ═══════════════════════════════════════════════════════════════
  // UNIT 6 — Asking for and giving recommendations
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-u6-37',
    word: 'it\'s probably worth',
    translation: 'Muhtemelen değer, yapılmaya değer',
    example: 'It\'s probably worth asking her where she wants to go.',
    category: 'phrase',
  },
  {
    id: 'v-u6-38',
    word: 'would you recommend',
    translation: 'Tavsiye eder misiniz?',
    example: 'Would you recommend buying a very expensive ring?',
    category: 'phrase',
  },
  {
    id: 'v-u6-39',
    word: 'it\'s much better to',
    translation: '... yapmak çok daha iyi',
    example: 'It\'s much better to buy something that\'s her style.',
    category: 'phrase',
  },
  {
    id: 'v-u6-40',
    word: 'it\'s not a good idea to',
    translation: '... yapmak iyi bir fikir değil',
    example: 'It\'s not a good idea to ask her what she likes.',
    category: 'phrase',
  },

  // ═══════════════════════════════════════════════════════════════
  // EXTENSION — General & Academic Vocabulary
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-ext-01',
    word: 'authority',
    translation: 'Yetki, otorite',
    example: 'He is a leading authority on climate change.',
    category: 'noun',
  },
  {
    id: 'v-ext-02',
    word: 'herb',
    translation: 'Bitki, baharat',
    example: 'I like to use fresh herbs when I cook Italian food.',
    category: 'noun',
  },
  {
    id: 'v-ext-03',
    word: 'mentioned',
    translation: 'Bahsedilen, sözü geçen',
    example: 'She mentioned that she might be late for the meeting.',
    category: 'verb',
  },
  {
    id: 'v-ext-04',
    word: 'passionate',
    translation: 'Tutkulu',
    example: 'He is very passionate about playing the piano.',
    category: 'adjective',
  },
  {
    id: 'v-ext-05',
    word: 'court',
    translation: 'Saha (spor) / Mahkeme',
    example: 'The tennis court is wet because of the rain.',
    category: 'noun',
  },
  {
    id: 'v-ext-06',
    word: 'cuisine',
    translation: 'Mutfak (kültürü)',
    example: 'Turkish cuisine is famous for its diverse flavors.',
    category: 'noun',
  },
  {
    id: 'v-ext-07',
    word: 'steam',
    translation: 'Buhar',
    example: 'Be careful, the steam from the kettle is very hot.',
    category: 'noun',
  },
  {
    id: 'v-ext-08',
    word: 'available',
    translation: 'Mevcut, müsait',
    example: 'Are there any tickets available for the concert?',
    category: 'adjective',
  },
  {
    id: 'v-ext-10',
    word: 'sharp',
    translation: 'Keskin',
    example: 'You need a sharp knife to cut the meat properly.',
    category: 'adjective',
  },
  {
    id: 'v-ext-11',
    word: 'ruin',
    translation: 'Mahvetmek, bozmak',
    example: 'Don\'t leave the bread out or the humidity will ruin it.',
    category: 'verb',
  },
  {
    id: 'v-ext-12',
    word: 'jewellery',
    translation: 'Mücevher',
    example: 'She wears a lot of expensive jewellery.',
    category: 'noun',
  },
  {
    id: 'v-ext-13',
    word: 'propose',
    translation: 'Evlenme teklifi etmek / Önermek',
    example: 'He decided to propose to her during their holiday in Paris.',
    category: 'verb',
  },
  {
    id: 'v-ext-14',
    word: 'intonation',
    translation: 'Vurgu, tonlama',
    example: 'His intonation was strange, so I didn\'t understand the question.',
    category: 'noun',
  },
  {
    id: 'v-ext-15',
    word: 'varied',
    translation: 'Çeşitli',
    example: 'The students have very varied backgrounds.',
    category: 'adjective',
  },
  {
    id: 'v-ext-16',
    word: 'deduction',
    translation: 'Çıkarımda bulunmak',
    example: 'Detectives use deduction to solve crimes.',
    category: 'noun',
  },
  {
    id: 'v-ext-17',
    word: 'privately',
    translation: 'Sana özel, özel olarak',
    example: 'Can we speak privately for a moment?',
    category: 'adverb',
  },
  {
    id: 'v-ext-18',
    word: 'review',
    translation: 'Yorum, inceleme',
    example: 'I read a positive review of the new movie.',
    category: 'noun',
  },
  {
    id: 'v-ext-19',
    word: 'charming',
    translation: 'Çarpıcı, büyüleyici',
    example: 'The hotel was small but very charming.',
    category: 'adjective',
  },
  {
    id: 'v-ext-20',
    word: 'cottage',
    translation: 'Kulübe, kır evi',
    example: 'They stayed in a small cottage by the lake.',
    category: 'noun',
  },
  {
    id: 'v-ext-21',
    word: 'property',
    translation: 'Mal, mülk',
    example: 'The building is private property, so you cannot enter.',
    category: 'noun',
  },
  {
    id: 'v-ext-22',
    word: 'crime',
    translation: 'Suç',
    example: 'The police are investigating a serious crime.',
    category: 'noun',
  },
  {
    id: 'v-ext-23',
    word: 'carbon footprint',
    translation: 'Karbon ayak izi',
    example: 'We should all try to reduce our carbon footprint.',
    category: 'phrase',
  },
  {
    id: 'v-ext-24',
    word: 'pleasure',
    translation: 'Zevk, memnuniyet',
    example: 'It was a pleasure to meet you.',
    category: 'noun',
  },
  {
    id: 'v-ext-25',
    word: 'rely',
    translation: 'Güvenmek',
    example: 'You can always rely on your best friend for help.',
    category: 'verb',
  },
  {
    id: 'v-ext-26',
    word: 'cope',
    translation: 'Başa çıkmak',
    example: 'It\'s hard to cope with a lot of stress at work.',
    category: 'verb',
  },
  {
    id: 'v-ext-27',
    word: 'warder',
    translation: 'Trafik polisi (görevli)',
    example: 'The traffic warder gave him a fine for parking illegally.',
    category: 'noun',
  },
  {
    id: 'v-ext-29',
    word: 'covering',
    translation: 'Kaplama',
    example: 'The table has a plastic covering to protect it.',
    category: 'noun',
  },
  {
    id: 'v-ext-30',
    word: 'put up',
    translation: 'Asmak, kurmak',
    example: 'They put up a new poster on the wall.',
    category: 'verb',
  },
  {
    id: 'v-ext-31',
    word: 'fool',
    translation: 'Aptal, kandırmak',
    example: 'Don\'t be a fool; you should listen to his advice.',
    category: 'noun',
  },
  {
    id: 'v-ext-32',
    word: 'fooled',
    translation: 'Kandırılmış',
    example: 'I was fooled by his clever disguise.',
    category: 'verb',
  },
  {
    id: 'v-ext-33',
    word: 'made up',
    translation: 'Uydurmak, uydurma',
    example: 'The story he told was completely made up.',
    category: 'phrase',
  },
  {
    id: 'v-ext-34',
    word: 'came across',
    translation: 'Karşılaşmak, rastlamak',
    example: 'I came across an old photo while cleaning my room.',
    category: 'verb',
  },
  {
    id: 'v-ext-35',
    word: 'deceive',
    translation: 'Aldatmak',
    example: 'He tried to deceive the customers with false promises.',
    category: 'verb',
  },
  {
    id: 'v-ext-36',
    word: 'on the whole',
    translation: 'Tümüyle, genel olarak',
    example: 'On the whole, I think the project was a success.',
    category: 'phrase',
  },
  {
    id: 'v-ext-37',
    word: 'tends to',
    translation: 'Eğilimli olmak',
    example: 'It tends to rain a lot in London during the spring.',
    category: 'phrase',
  },
  {
    id: 'v-ext-38',
    word: 'vague',
    translation: 'Başıboş, belirsiz',
    example: 'His explanation was very vague, so I\'m still confused.',
    category: 'adjective',
  },
  {
    id: 'v-ext-39',
    word: 'stuff like that',
    translation: 'Öyle şeyler, ve benzeri',
    example: 'I like reading novels, biographies, and stuff like that.',
    category: 'phrase',
  },
  {
    id: 'v-ext-40',
    word: 'newsfeed',
    translation: 'Haber akışı',
    example: 'I check my social media newsfeed every morning.',
    category: 'noun',
  },
  {
    id: 'v-ext-41',
    word: 'label',
    translation: 'Etiket',
    example: 'Check the label on the bottle to see the ingredients.',
    category: 'noun',
  },
  {
    id: 'v-ext-42',
    word: 'binge watching',
    translation: 'Art arda izlemek',
    example: 'I spent the whole weekend binge watching my favorite series.',
    category: 'phrase',
  },
  {
    id: 'v-ext-43',
    word: 'fascinated',
    translation: 'Büyülenmiş',
    example: 'She was fascinated by the history of ancient Egypt.',
    category: 'adjective',
  },
  {
    id: 'v-ext-44',
    word: 'cliffhanger',
    translation: 'Uçurum (heyecanlı son)',
    example: 'The movie ended with a huge cliffhanger, so I want to see the sequel.',
    category: 'noun',
  },
  {
    id: 'v-ext-45',
    word: 'streaming services',
    translation: 'Dijital platform (yayın)',
    example: 'Netflix is one of the most popular streaming services.',
    category: 'phrase',
  },
  {
    id: 'v-ext-46',
    word: 'release',
    translation: 'Piyasaya sürmek, serbest bırakmak',
    example: 'The company will release a new product next month.',
    category: 'verb',
  },
  {
    id: 'v-ext-47',
    word: 'pleasant',
    translation: 'Hoş',
    example: 'We had a very pleasant evening at the restaurant.',
    category: 'adjective',
  },
  {
    id: 'v-ext-48',
    word: 'trouble',
    translation: 'Bela, sorun',
    example: 'I\'m sorry to cause you so much trouble.',
    category: 'noun',
  },
  {
    id: 'v-ext-49',
    word: 'audience',
    translation: 'Dinleyici, seyirci',
    example: 'The audience cheered at the end of the performance.',
    category: 'noun',
  },
  {
    id: 'v-ext-50',
    word: 'held',
    translation: 'Tutulmuş, düzenlenmiş',
    example: 'The meeting was held in the conference room.',
    category: 'verb',
  },
  {
    id: 'v-ext-51',
    word: 'stage',
    translation: 'Sahne',
    example: 'The actors were nervous before going on stage.',
    category: 'noun',
  },
  {
    id: 'v-ext-52',
    word: 'recommendation',
    translation: 'Öneri',
    example: 'Can you give me a recommendation for a good book?',
    category: 'noun',
  },
  {
    id: 'v-ext-53',
    word: 'respond',
    translation: 'Cevap vermek',
    example: 'Please respond to the email as soon as possible.',
    category: 'verb',
  },
  {
    id: 'v-ext-54',
    word: 'hang on',
    translation: 'Bekle',
    example: 'Hang on a second, I\'ll be right back.',
    category: 'verb',
  },
  {
    id: 'v-ext-55',
    word: 'nominate',
    translation: 'Aday gösterilmek',
    example: 'The director was nominated for an Oscar.',
    category: 'verb',
  },
  {
    id: 'v-ext-56',
    word: 'gown',
    translation: 'Cübbe, abiye',
    example: 'She wore a beautiful evening gown to the party.',
    category: 'noun',
  },
  {
    id: 'v-ext-57',
    word: 'powder',
    translation: 'Pudra',
    example: 'She put some powder on her face before going out.',
    category: 'noun',
  },
  {
    id: 'v-ext-58',
    word: 'proud of',
    translation: 'Gurur duymak',
    example: 'Her parents are very proud of her achievements.',
    category: 'phrase',
  },
  {
    id: 'v-ext-59',
    word: 'essential',
    translation: 'Gerekli, temel',
    example: 'Water is essential for all living things.',
    category: 'adjective',
  },
  {
    id: 'v-ext-60',
    word: 'making the most of',
    translation: 'En iyi şekilde yararlanmak',
    example: 'We are making the most of the sunny weather by going to the beach.',
    category: 'phrase',
  },
  {
    id: 'v-ext-61',
    word: 'serendipity',
    translation: 'Tesadüf (mutlu)',
    example: 'Finding that old book was a piece of pure serendipity.',
    category: 'noun',
  },
  {
    id: 'v-ext-62',
    word: 'raise',
    translation: 'Kaldırmak, artırmak',
    example: 'The company decided to raise the prices of its products.',
    category: 'verb',
  },
  {
    id: 'v-ext-63',
    word: 'actually',
    translation: 'Aslında',
    example: 'I actually liked the movie more than I expected.',
    category: 'adverb',
  },
  {
    id: 'v-ext-64',
    word: 'take part',
    translation: 'Yer almak',
    example: 'Thousands of people take part in the marathon every year.',
    category: 'verb',
  },
  {
    id: 'v-ext-65',
    word: 'currently',
    translation: 'Şu sıralar',
    example: 'I am currently working on a new project.',
    category: 'adverb',
  },
  {
    id: 'v-ext-66',
    word: 'chop',
    translation: 'Doğramak',
    example: 'Could you help me chop the vegetables for the salad?',
    category: 'verb',
  },
  {
    id: 'v-ext-67',
    word: 'mash',
    translation: 'Ezmek',
    example: 'You should mash the potatoes with a little milk and butter.',
    category: 'verb',
  },
  {
    id: 'v-ext-68',
    word: 'squeeze',
    translation: 'Sıkmak',
    example: 'Squeeze some lemon juice over the fish.',
    category: 'verb',
  },
  {
    id: 'v-ext-69',
    word: 'stir',
    translation: 'Karıştırmak',
    example: 'Stir the soup constantly so it doesn\'t burn.',
    category: 'verb',
  },
  {
    id: 'v-ext-70',
    word: 'minced',
    translation: 'Kıyma, kıyılmış',
    example: 'We need some minced meat to make the lasagna.',
    category: 'adjective',
  },
  {
    id: 'v-ext-71',
    word: 'heat up',
    translation: 'Isıtmak',
    example: 'I\'ll heat up the leftovers for dinner.',
    category: 'verb',
  },
  {
    id: 'v-ext-72',
    word: 'attic',
    translation: 'Tavan arası',
    example: 'We keep our old suitcases in the attic.',
    category: 'noun',
  },
  {
    id: 'v-ext-73',
    word: 'steps',
    translation: 'Basmak, adımlar',
    example: 'Be careful as you go down the steps.',
    category: 'noun',
  },
  {
    id: 'v-ext-74',
    word: 'basement',
    translation: 'Bodrum',
    example: 'The laundry room is located in the basement.',
    category: 'noun',
  },
  {
    id: 'v-ext-75',
    word: 'flyer',
    translation: 'El ilanı',
    example: 'A man was giving out flyers for the new gym.',
    category: 'noun',
  },
  {
    id: 'v-ext-76',
    word: 'brainstorm',
    translation: 'Beyin fırtınası yapmak',
    example: 'The team met to brainstorm ideas for the advertisement.',
    category: 'verb',
  },
  {
    id: 'v-ext-77',
    word: 'subscribe',
    translation: 'Abone olmak',
    example: 'Don\'t forget to subscribe to my channel for more videos.',
    category: 'verb',
  },
  {
    id: 'v-ext-78',
    word: 'beat',
    translation: 'Yenmek',
    example: 'Our team managed to beat the champions in the final.',
    category: 'verb',
  },
  {
    id: 'v-ext-79',
    word: 'competitor',
    translation: 'Yarışmacı, rakip',
    example: 'The company is trying to stay ahead of its competitors.',
    category: 'noun',
  },
  {
    id: 'v-ext-80',
    word: 'net',
    translation: 'File',
    example: 'The ball went over the net and landed out of bounds.',
    category: 'noun',
  },
  {
    id: 'v-ext-81',
    word: 'opponents',
    translation: 'Rakip',
    example: 'He respected his opponents but was determined to win.',
    category: 'noun',
  },
  {
    id: 'v-ext-82',
    word: 'track',
    translation: 'Pist, yol',
    example: 'The runners are waiting at the start of the track.',
    category: 'noun',
  },
  {
    id: 'v-ext-83',
    word: 'make sense',
    translation: 'Anlamına gelmek, mantıklı olmak',
    example: 'His explanation doesn\'t make sense to me.',
    category: 'phrase',
  },
  {
    id: 'v-ext-84',
    word: 'among',
    translation: 'Arasında',
    example: 'The secret was shared among the members of the group.',
    category: 'preposition',
  },
  {
    id: 'v-ext-85',
    word: 'shallow',
    translation: 'Sığ',
    example: 'The water is very shallow here, so you can\'t dive.',
    category: 'adjective',
  },
  {
    id: 'v-ext-86',
    word: 'plenty',
    translation: 'Bol bol',
    example: 'Don\'t worry, we have plenty of time to finish.',
    category: 'adverb',
  },
  {
    id: 'v-ext-87',
    word: 'none of',
    translation: 'Hiçbiri',
    example: 'None of the students knew the answer to the question.',
    category: 'phrase',
  },
  {
    id: 'v-ext-88',
    word: 'wire',
    translation: 'Tel',
    example: 'The electrical wire was damaged, so the lamp didn\'t work.',
    category: 'noun',
  },
  {
    id: 'v-ext-89',
    word: 'bunker',
    translation: 'Sığınak',
    example: 'They built a bunker to protect themselves during the war.',
    category: 'noun',
  },
  {
    id: 'v-ext-91',
    word: 'assume',
    translation: 'Zannetmek, varsaymak',
    example: 'Don\'t assume that everyone knows the rules.',
    category: 'verb',
  },
  {
    id: 'v-ext-92',
    word: 'permission',
    translation: 'İzin',
    example: 'You need permission from your parents to go on the trip.',
    category: 'noun',
  },
  {
    id: 'v-ext-93',
    word: 'unfamiliar',
    translation: 'Tanımadık, aşina olunmayan',
    example: 'I felt lost in the unfamiliar city.',
    category: 'adjective',
  },
  {
    id: 'v-ext-94',
    word: 'frustration',
    translation: 'Hayal kırıklığı',
    example: 'He sighed in frustration when the computer crashed again.',
    category: 'noun',
  },
  {
    id: 'v-ext-95',
    word: 'struggle',
    translation: 'Mücadele etmek',
    example: 'She had to struggle to finish the race.',
    category: 'verb',
  },
  {
    id: 'v-ext-96',
    word: 'afraid',
    translation: 'Endişe, korkmak',
    example: 'Are you afraid of spiders?',
    category: 'adjective',
  },
  {
    id: 'v-ext-97',
    word: 'reduce',
    translation: 'Azaltmak',
    example: 'The government is trying to reduce the amount of waste.',
    category: 'verb',
  },
  {
    id: 'v-ext-98',
    word: 'appreciate',
    translation: 'Takdir etmek',
    example: 'I really appreciate all the help you\'ve given me.',
    category: 'verb',
  },
  {
    id: 'v-ext-99',
    word: 'whether',
    translation: 'İkisinden biri, olup olmadığı',
    example: 'I don\'t know whether he will come or not.',
    category: 'conjunction',
  },
  {
    id: 'v-ext-100',
    word: 'conflict',
    translation: 'Anlaşmazlık',
    example: 'The two countries are trying to resolve their conflict.',
    category: 'noun',
  },
  {
    id: 'v-ext-101',
    word: 'likewise',
    translation: 'Benzer bir şekilde',
    example: 'He is a hard worker, and his brother is likewise.',
    category: 'adverb',
  },
  {
    id: 'v-ext-102',
    word: 'smooth',
    translation: 'Pürüzsüz',
    example: 'The surface of the lake was perfectly smooth.',
    category: 'adjective',
  },
  {
    id: 'v-ext-103',
    word: 'clarifying',
    translation: 'Açıklığa kavuşturmak',
    example: 'He spent some time clarifying his position on the issue.',
    category: 'verb',
  },
  {
    id: 'v-ext-104',
    word: 'awareness',
    translation: 'Farkındalık',
    example: 'The campaign aims to raise awareness of mental health issues.',
    category: 'noun',
  },
  {
    id: 'v-ext-105',
    word: 'interaction',
    translation: 'Etkileşim',
    example: 'The interaction between the two groups was very positive.',
    category: 'noun',
  },
  {
    id: 'v-ext-106',
    word: 'issue',
    translation: 'Sorun, mesele',
    example: 'Climate change is a global issue that affects everyone.',
    category: 'noun',
  },
  {
    id: 'v-ext-107',
    word: 'obstacle',
    translation: 'Engel',
    example: 'The main obstacle to success is the lack of funding.',
    category: 'noun',
  },
  {
    id: 'v-ext-108',
    word: 'concern',
    translation: 'Endişe, kaygı',
    example: 'His health is a major concern for his family.',
    category: 'noun',
  },
  {
    id: 'v-ext-109',
    word: 'contribute to',
    translation: 'Katkıda bulunmak',
    example: 'Smoking can contribute to a variety of health problems.',
    category: 'verb',
  },
  {
    id: 'v-ext-110',
    word: 'congestion',
    translation: 'Tıkanıklık',
    example: 'The city is suffering from severe traffic congestion.',
    category: 'noun',
  },
  {
    id: 'v-ext-111',
    word: 'overcome',
    translation: 'Üstesinden gelmek',
    example: 'She managed to overcome her fear of heights.',
    category: 'verb',
  },
  {
    id: 'v-ext-112',
    word: 'prevent',
    translation: 'Önlemek',
    example: 'The security measures are designed to prevent accidents.',
    category: 'verb',
  },
  {
    id: 'v-ext-113',
    word: 'schedule',
    translation: 'Program',
    example: 'I have a very busy schedule this week.',
    category: 'noun',
  },
  {
    id: 'v-ext-114',
    word: 'amount',
    translation: 'Miktar',
    example: 'The amount of data we have is increasing every day.',
    category: 'noun',
  },
  {
    id: 'v-ext-115',
    word: 'summarize',
    translation: 'Özetlemek',
    example: 'Can you summarize the main points of the article?',
    category: 'verb',
  },
  {
    id: 'v-ext-116',
    word: 'deter',
    translation: 'Caydırmak',
    example: 'The high price might deter some people from buying it.',
    category: 'verb',
  },
  {
    id: 'v-ext-117',
    word: 'promotion',
    translation: 'Terfi',
    example: 'She got a promotion after working hard for two years.',
    category: 'noun',
  },
  {
    id: 'v-ext-118',
    word: 'stuck',
    translation: 'Sıkışmak, takılmak',
    example: 'The car got stuck in the mud.',
    category: 'adjective',
  },
  {
    id: 'v-ext-119',
    word: 'deadline',
    translation: 'Zaman sınırı, teslim tarihi',
    example: 'We need to finish the project before the deadline.',
    category: 'noun',
  },
  {
    id: 'v-ext-120',
    word: 'overcharged',
    translation: 'Kazıklanmak, fazla ücret almak',
    example: 'I think I was overcharged for the meal.',
    category: 'verb',
  },
  {
    id: 'v-ext-121',
    word: 'get in touch',
    translation: 'Temas kurmak',
    example: "It was great to get in touch with my old school friends.",
    category: 'phrase',
  },
  {
    id: 'v-ext-122',
    word: 'express your feelings',
    translation: 'Hislerini ifade etmek',
    example: "Some people find it difficult to express their feelings.",
    category: 'phrase',
  },
  {
    id: 'v-ext-123',
    word: 'reached out',
    translation: 'Ulaşmak, iletişime geçmek',
    example: "I reached out to her, but she didn't respond.",
    category: 'verb',
  },
  {
    id: 'v-ext-124',
    word: 'interact with',
    translation: 'Etkileşim kurmak',
    example: "The children were encouraged to interact with each other.",
    category: 'verb',
  },
  {
    id: 'v-ext-125',
    word: 'put into words',
    translation: 'Kelimeye dökmek',
    example: "I can't put my gratitude into words.",
    category: 'phrase',
  },
  {
    id: 'v-ext-126',
    word: 'give your opinion',
    translation: 'Fikir vermek',
    example: "Don't be afraid to give your opinion during the meeting.",
    category: 'phrase',
  },
  {
    id: 'v-ext-127',
    word: 'argue',
    translation: 'Tartışmak',
    example: "They often argue about money.",
    category: 'verb',
  },
  {
    id: 'v-ext-128',
    word: 'complaint',
    translation: 'Şikayet',
    example: "We received a formal complaint about the noise.",
    category: 'noun',
  },
  {
    id: 'v-ext-129',
    word: 'encourage',
    translation: 'Cesaret vermek, teşvik etmek',
    example: "My parents always encourage me to try new things.",
    category: 'verb',
  },
  {
    id: 'v-ext-130',
    word: 'persuade',
    translation: 'İkna etmek',
    example: "I managed to persuade him to come to the party.",
    category: 'verb',
  },
  {
    id: 'v-ext-131',
    word: 'insist',
    translation: 'Israr etmek',
    example: "She insisted on paying for the meal.",
    category: 'verb',
  },
  {
    id: 'v-ext-132',
    word: 'greet',
    translation: 'Selamlamak',
    example: "He greeted us with a warm smile.",
    category: 'verb',
  },
  {
    id: 'v-ext-133',
    word: 'admit',
    translation: 'İtiraf etmek',
    example: "I have to admit that I was wrong.",
    category: 'verb',
  },
  {
    id: 'v-ext-134',
    word: 'forgive',
    translation: 'Affetmek',
    example: "It's hard to forgive someone who has lied to you.",
    category: 'verb',
  },
  {
    id: 'v-ext-135',
    word: 'refuse',
    translation: 'Reddetmek',
    example: "He refused to answer the question.",
    category: 'verb',
  },
  {
    id: 'v-ext-136',
    word: 'update',
    translation: 'Güncellemek',
    example: "You need to update your password regularly.",
    category: 'verb',
  },
  {
    id: 'v-ext-137',
    word: 'awful',
    translation: 'Berbat, çok kötü (Extreme: bad)',
    example: "The weather was awful all weekend.",
    category: 'adjective',
  },
  {
    id: 'v-ext-138',
    word: 'filthy',
    translation: 'Çok kirli (Extreme: dirty)',
    example: "Wash your hands! They are filthy.",
    category: 'adjective',
  },
  {
    id: 'v-ext-139',
    word: 'boiling',
    translation: 'Çok sıcak (Extreme: hot)',
    example: "It's boiling in this room. Can we open a window?",
    category: 'adjective',
  },
  {
    id: 'v-ext-140',
    word: 'freezing',
    translation: 'Çok soğuk (Extreme: cold)',
    example: "I'm freezing. I should have worn a coat.",
    category: 'adjective',
  },
  {
    id: 'v-ext-141',
    word: 'brilliant',
    translation: 'Harika, çok iyi (Extreme: good)',
    example: "That's a brilliant idea!",
    category: 'adjective',
  },
  {
    id: 'v-ext-142',
    word: 'furious',
    translation: 'Çok kızgın (Extreme: angry)',
    example: "He was furious when he found out the truth.",
    category: 'adjective',
  },
  {
    id: 'v-ext-143',
    word: 'delicious',
    translation: 'Çok lezzetli (Extreme: tasty)',
    example: "The meal was absolutely delicious.",
    category: 'adjective',
  },
  {
    id: 'v-ext-144',
    word: 'miserable',
    translation: 'Çok mutsuz (Extreme: unhappy)',
    example: "She felt miserable after failing the exam.",
    category: 'adjective',
  },
  {
    id: 'v-ext-145',
    word: 'enormous',
    translation: 'Kocaman, devasa (Extreme: big)',
    example: "The house was enormous, with ten bedrooms.",
    category: 'adjective',
  },
  {
    id: 'v-ext-147',
    word: 'criticize',
    translation: 'Eleştirmek',
    example: "He is always criticizing other people.",
    category: 'verb',
  },
  {
    id: 'v-ext-148',
    word: 'exists',
    translation: 'Var olmak',
    example: "Do you believe that life exists on other planets?",
    category: 'verb',
  },
  {
    id: 'v-ext-149',
    word: 'memorize',
    translation: 'Ezberlemek',
    example: "I need to memorize all these words for the test.",
    category: 'verb',
  },
  {
    id: 'v-ext-150',
    word: 'challenge',
    translation: 'Zorluk, meydan okuma',
    example: "Learning a new language is a big challenge.",
    category: 'noun',
  },
  {
    id: 'v-ext-152',
    word: 'translate',
    translation: 'Çevirmek',
    example: "Can you translate this sentence into Turkish?",
    category: 'verb',
  },
  {
    id: 'v-ext-153',
    word: 'fluently',
    translation: 'Akıcı bir şekilde',
    example: "She speaks English fluently.",
    category: 'adverb',
  },
  {
    id: 'v-ext-154',
    word: 'remind',
    translation: 'Hatırlatmak',
    example: "Please remind me to call him later.",
    category: 'verb',
  },
  {
    id: 'v-ext-155',
    word: 'master',
    translation: 'Bir konuda uzmanlaşmak, tam halletmek',
    example: "It takes years to master a musical instrument.",
    category: 'verb',
  },
  {
    id: 'v-ext-156',
    word: 'make progress',
    translation: 'İlerleme kaydetmek',
    example: "He is making steady progress in his studies.",
    category: 'phrase',
  },
  {
    id: 'v-ext-157',
    word: 'me too',
    translation: 'Ben de (olumlu cümlelere katılırken)',
    example: '"I like ice cream." - "Me too."',
    category: 'phrase',
  },
  {
    id: 'v-ext-158',
    word: 'me neither',
    translation: 'Ben de (olumsuz cümlelere katılırken)',
    example: `\"I don't like spicy food.\" - \"Me neither.\"`,
    category: 'phrase',
  },
  {
    id: 'v-ext-159',
    word: 'applied for',
    translation: 'Başvurmak (iş, okul vb.)',
    example: "I've applied for a job as a translator.",
    category: 'verb',
  },
  {
    id: 'v-ext-160',
    word: 'candidate',
    translation: 'Aday',
    example: "There are several candidates for the position.",
    category: 'noun',
  },
  {
    id: 'v-ext-161',
    word: 'knowledge',
    translation: 'Bilgi',
    example: "He has a broad knowledge of history.",
    category: 'noun',
  },
  {
    id: 'v-ext-162',
    word: 'career',
    translation: 'Kariyer',
    example: "She had a successful career as a lawyer.",
    category: 'noun',
  },
  {
    id: 'v-ext-163',
    word: 'business contact',
    translation: 'İş bağlantısı',
    example: "He has a lot of useful business contacts.",
    category: 'noun',
  },
  {
    id: 'v-ext-164',
    word: 'employee',
    translation: 'Çalışan, işçi',
    example: "The company has over 500 employees.",
    category: 'noun',
  },
  {
    id: 'v-ext-165',
    word: 'in charge of',
    translation: 'Sorumlu (bir işten)',
    example: "Who is in charge of this department?",
    category: 'phrase',
  },
  {
    id: 'v-ext-166',
    word: 'make another',
    translation: 'Bir başkasını yapmak',
    example: "If you don't like this one, I can make another.",
    category: 'phrase',
  },
  {
    id: 'v-ext-167',
    word: 'CV',
    translation: 'Özgeçmiş',
    example: "Please send your CV and a covering letter.",
    category: 'noun',
  },
  {
    id: 'v-ext-168',
    word: 'employer',
    translation: 'İşveren',
    example: "My employer is very understanding.",
    category: 'noun',
  },
  {
    id: 'v-ext-169',
    word: 'freeze',
    translation: 'Donmak (bilgisayar vb.)',
    example: "My computer keeps freezing.",
    category: 'verb',
  },
  {
    id: 'v-ext-170',
    word: 'install',
    translation: 'Yüklemek, kurmak',
    example: "I need to install a new software.",
    category: 'verb',
  },
  {
    id: 'v-ext-173',
    word: 'upload',
    translation: 'Yüklemek (internete vb.)',
    example: "I am uploading the photos to the cloud.",
    category: 'verb',
  },
  {
    id: 'v-ext-174',
    word: 'download',
    translation: 'İndirmek',
    example: "You can download the app for free.",
    category: 'verb',
  },
  {
    id: 'v-ext-175',
    word: 'flexibility',
    translation: 'Esneklik',
    example: "I enjoy the flexibility of working from home.",
    category: 'noun',
  },
  {
    id: 'v-ext-176',
    word: 'properly',
    translation: 'Düzgün bir şekilde',
    example: "Make sure you do it properly.",
    category: 'adverb',
  },

  {
    id: 'successful',
    word: 'successful',
    translation: 'başarılı',
    example: 'She became a very successful entrepreneur in a short time. - Kısa sürede çok başarılı bir girişimci oldu.',
    category: 'phrase'
  },
  {
    id: 'talented',
    word: 'talented',
    translation: 'yetenekli',
    example: 'He is a talented musician who plays three different instruments. - Üç farklı enstrüman çalan yetenekli bir müzisyendir.',
    category: 'phrase'
  },
  {
    id: 'intelligent',
    word: 'intelligent',
    translation: 'zeki',
    example: 'Dolphins are known to be highly intelligent animals. - Yunusların son derece zeki hayvanlar olduğu bilinmektedir.',
    category: 'phrase'
  },
  {
    id: 'confident',
    word: 'confident',
    translation: 'öz güvenli',
    example: 'You need to be confident during your job interview. - İş görüşmende öz güvenli olman gerekiyor.',
    category: 'phrase'
  },
  {
    id: 'determined',
    word: 'determined',
    translation: 'kararlı',
    example: 'Despite the difficulties, she remained determined to win the race. - Zorluklara rağmen, yarışı kazanmakta kararlı kaldı.',
    category: 'phrase'
  },
  {
    id: 'achievement',
    word: 'achievement',
    translation: 'başarı',
    example: 'Graduating from university was her greatest achievement. - Üniversiteden mezun olmak onun en büyük başarısıydı.',
    category: 'phrase'
  },
  {
    id: 'attitude',
    word: 'attitude',
    translation: 'tutum',
    example: 'Having a positive attitude can change your whole life. - Pozitif bir tutuma sahip olmak tüm hayatınızı değiştirebilir.',
    category: 'phrase'
  },
  {
    id: 'give-up',
    word: 'give up',
    translation: 'vazgeçmek',
    example: 'No matter how hard it gets, you should never give up. - Ne kadar zorlaşırsa zorlaşsın, asla vazgeçmemelisin.',
    category: 'phrase'
  },
  {
    id: 'sociable',
    word: 'sociable',
    translation: 'sosyal',
    example: 'My sister is very sociable and loves meeting new people. - Kız kardeşim çok sosyaldir ve yeni insanlarla tanışmayı sever.',
    category: 'phrase'
  },
  {
    id: 'outgoing',
    word: 'outgoing',
    translation: 'uyumlu',
    example: 'He has an outgoing personality and easily makes friends. - Uyumlu ve dışa dönük bir kişiliği var ve kolayca arkadaş edinir.',
    category: 'phrase'
  },
  {
    id: 'talkative',
    word: 'talkative',
    translation: 'konuşkan',
    example: 'She is usually quiet, but she gets talkative when she is excited. - Genelde sessizdir ama heyecanlandığında konuşkan olur.',
    category: 'phrase'
  },
  {
    id: 'shy',
    word: 'shy',
    translation: 'utangaç',
    example: 'The little boy was too shy to speak to the guests. - Küçük çocuk misafirlerle konuşamayacak kadar utangaçtı.',
    category: 'phrase'
  },
  {
    id: 'reserved',
    word: 'reserved',
    translation: 'duygu belli etmeyen',
    example: 'He is a reserved person who rarely shares his feelings. - Duygularını nadiren paylaşan, duygu belli etmeyen biridir.',
    category: 'phrase'
  },
  {
    id: 'lively',
    word: 'lively',
    translation: 'canlı',
    example: 'The city center is always lively and full of energy. - Şehir merkezi her zaman canlı ve enerji doludur.',
    category: 'phrase'
  },
  {
    id: 'serious',
    word: 'serious',
    translation: 'ciddi',
    example: 'We need to have a serious conversation about your future. - Geleceğin hakkında ciddi bir konuşma yapmamız gerekiyor.',
    category: 'phrase'
  },
  {
    id: 'sensitive',
    word: 'sensitive',
    translation: 'hassas',
    example: 'My skin is very sensitive to sunlight. - Cildim güneş ışığına karşı çok hassas.',
    category: 'phrase'
  },
  {
    id: 'valued',
    word: 'valued',
    translation: 'değerli',
    example: 'As a member of this team, your opinions are highly valued. - Bu takımın bir üyesi olarak, fikirleriniz oldukça değerlidir.',
    category: 'phrase'
  },
  {
    id: 'movement',
    word: 'movement',
    translation: 'eylem',
    example: 'There is a growing movement to protect the environment. - Çevreyi korumak için büyüyen bir eylem var.',
    category: 'phrase'
  },
  {
    id: 'despite',
    word: 'despite',
    translation: 'rağmen',
    example: 'He went to work despite being very ill. - Çok hasta olmasına rağmen işe gitti.',
    category: 'phrase'
  },
  {
    id: 'songwriters',
    word: 'songwriters',
    translation: 'söz yazarı',
    example: 'The most successful songwriters often write from their own experiences. - En başarılı söz yazarları genellikle kendi deneyimlerinden yazarlar.',
    category: 'phrase'
  },
  {
    id: 'bright',
    word: 'bright',
    translation: 'zeki',
    example: 'She is a bright student who always gets the highest grades. - O, her zaman en yüksek notları alan zeki bir öğrencidir.',
    category: 'phrase'
  },
  {
    id: 'towards',
    word: 'towards',
    translation: 'karşı',
    example: 'The dog ran towards me as soon as I opened the gate. - Kapıyı açar açmaz köpek bana karşı koştu.',
    category: 'phrase'
  },
  {
    id: 'ability',
    word: 'ability',
    translation: 'yetenek',
    example: 'He has the ability to learn new languages very quickly. - Yeni dilleri çok hızlı öğrenme yeteneğine sahiptir.',
    category: 'phrase'
  },
  {
    id: 'pick-up',
    word: 'pick up',
    translation: 'almak, teslim almak',
    example: 'Can you pick up the package from the post office? - Postaneden paketi teslim alabilir misin?',
    category: 'phrase'
  },
  {
    id: 'pollution',
    word: 'pollution',
    translation: 'kirlilik',
    example: 'Air pollution is a major problem in industrial cities. - Hava kirliliği sanayi şehirlerinde büyük bir sorundur.',
    category: 'phrase'
  },
  {
    id: 'endangered',
    word: 'endangered',
    translation: 'tehlikede',
    example: 'Pandas are considered an endangered species. - Pandalar tehlikede olan bir tür olarak kabul edilir.',
    category: 'phrase'
  },
  {
    id: 'damaged',
    word: 'damaged',
    translation: 'zarar görmüş',
    example: 'The storm damaged several houses in the neighborhood. - Fırtına mahalledeki birkaç eve zarar verdi.',
    category: 'phrase'
  },
  {
    id: 'protect',
    word: 'protect',
    translation: 'korumak',
    example: 'We must protect the forests for future generations. - Gelecek nesiller için ormanları korumalıyız.',
    category: 'phrase'
  },
  {
    id: 'environment',
    word: 'environment',
    translation: 'çevre',
    example: 'It is our responsibility to take care of the environment. - Çevreye iyi bakmak bizim sorumluluğumuzdur.',
    category: 'phrase'
  },
  {
    id: 'creature',
    word: 'creature',
    translation: 'yaratık',
    example: 'The deep ocean is home to many strange creatures. - Derin okyanus birçok tuhaf yaratığa ev sahipliği yapar.',
    category: 'phrase'
  },
  {
    id: 'destroy',
    word: 'destroy',
    translation: 'yerle bir etmek',
    example: 'The earthquake could destroy the entire city. - Deprem tüm şehri yerle bir edebilir.',
    category: 'phrase'
  },
  {
    id: 'species',
    word: 'species',
    translation: 'tür',
    example: 'There are thousands of different species of birds in the world. - Dünyada binlerce farklı kuş türü bulunmaktadır.',
    category: 'phrase'
  },
  {
    id: 'survive',
    word: 'survive',
    translation: 'hayatta kalmak',
    example: 'Camels can survive in the desert without water for days. - Develer çölde günlerce susuz hayatta kalabilirler.',
    category: 'phrase'
  },
  {
    id: 'limit',
    word: 'limit',
    translation: 'sınırlamak',
    example: 'We should limit the amount of sugar we eat every day. - Her gün yediğimiz şeker miktarını sınırlamalıyız.',
    category: 'phrase'
  },
  {
    id: 'branch',
    word: 'branch',
    translation: 'dal, branş',
    example: 'The bird built its nest on a high branch of the tree. - Kuş yuvasını ağacın yüksek bir dalına yaptı.',
    category: 'phrase'
  },
  {
    id: 'petals',
    word: 'petals',
    translation: 'taç yaprak',
    example: 'The rose had beautiful red petals. - Gülün güzel kırmızı taç yaprakları vardı.',
    category: 'phrase'
  },
  {
    id: 'feathers',
    word: 'feathers',
    translation: 'tüy',
    example: 'The parrot has brightly colored feathers. - Papağanın parlak renkli tüyleri vardır.',
    category: 'phrase'
  },
  {
    id: 'scales',
    word: 'scales',
    translation: 'pul',
    example: 'Fish are covered in scales to protect their bodies. - Balıklar vücutlarını korumak için pullarla kaplıdır.',
    category: 'phrase'
  },
  {
    id: 'fur',
    word: 'fur',
    translation: 'kürk',
    example: 'Cats have soft fur that keeps them warm in winter. - Kedilerin kışın onları sıcak tutan yumuşak kürkleri vardır.',
    category: 'phrase'
  },
  {
    id: 'bay',
    word: 'bay',
    translation: 'koy',
    example: 'They rented a small boat to sail around the bay. - Koyun etrafında yelken açmak için küçük bir tekne kiraladılar.',
    category: 'phrase'
  },
  {
    id: 'waterfall',
    word: 'waterfall',
    translation: 'şelale',
    example: 'We hiked for hours just to see the beautiful waterfall. - Sadece güzel şelaleyi görmek için saatlerce yürüdük.',
    category: 'phrase'
  },
  {
    id: 'cave',
    word: 'cave',
    translation: 'mağara',
    example: 'The explorers found ancient drawings inside the dark cave. - Kaşifler karanlık mağaranın içinde antik çizimler buldular.',
    category: 'phrase'
  },
  {
    id: 'paws',
    word: 'paws',
    translation: 'pati',
    example: 'The dog left muddy paws all over the clean floor. - Köpek temiz zeminin her yerinde çamurlu pati izleri bıraktı.',
    category: 'phrase'
  },
  {
    id: 'web',
    word: 'web',
    translation: 'ağ',
    example: 'The spider spun a complex web in the corner of the room. - Örümcek odanın köşesinde karmaşık bir ağ ördü.',
    category: 'phrase'
  },
  {
    id: 'hook',
    word: 'hook',
    translation: 'kanca',
    example: 'He used a sharp hook to catch the large fish. - Büyük balığı yakalamak için keskin bir kanca kullandı.',
    category: 'phrase'
  },
  {
    id: 'sticking',
    word: 'sticking',
    translation: 'yapışmak',
    example: 'The glue is sticking the pieces of paper together perfectly. - Yapıştırıcı kağıt parçalarını birbirine mükemmel bir şekilde yapıştırıyor.',
    category: 'phrase'
  },
  {
    id: 'arctic',
    word: 'arctic',
    translation: 'kutup',
    example: 'Polar bears live in the freezing cold conditions of the arctic. - Kutup ayıları kutbun dondurucu soğuk koşullarında yaşarlar.',
    category: 'phrase'
  },
  {
    id: 'inspire',
    word: 'inspire',
    translation: 'ilham almak',
    example: 'Her speech will inspire many young artists to follow their dreams. - Onun konuşması birçok genç sanatçıya hayallerinin peşinden gitmeleri için ilham alacaktır.',
    category: 'phrase'
  },
  {
    id: 'seeds',
    word: 'seeds',
    translation: 'tohum',
    example: 'We planted some seeds in the garden and waited for them to grow. - Bahçeye biraz tohum ektik ve büyümelerini bekledik.',
    category: 'phrase'
  },
  {
    id: 'rose',
    word: 'rose',
    translation: 'gül',
    example: 'He gave her a single red rose on their anniversary. - Yıl dönümlerinde ona tek bir kırmızı gül verdi.',
    category: 'phrase'
  },
  {
    id: 'pine-tree',
    word: 'pine tree',
    translation: 'çam ağacı',
    example: 'The forest was full of tall pine trees that smelled wonderful. - Orman harika kokan uzun çam ağaçlarıyla doluydu.',
    category: 'phrase'
  },
  {
    id: 'souvenir-shop',
    word: 'souvenir shop',
    translation: 'hediyelik eşya dükkanı',
    example: 'We bought some gifts for our friends from the souvenir shop. - Hediyelik eşya dükkanından arkadaşlarımız için bazı hediyeler aldık.',
    category: 'phrase'
  },
  {
    id: 'bottle-opener',
    word: 'bottle opener',
    translation: 'şişe açacağı',
    example: 'Can you pass me the bottle opener so I can open this drink? - Bu içeceği açabilmem için bana şişe açacağını uzatır mısın?',
    category: 'phrase'
  },
  {
    id: 'window-seat',
    word: 'window seat',
    translation: 'cam kenarı',
    example: 'I always request a window seat when I fly so I can see the clouds. - Uçtuğumda her zaman cam kenarı isterim böylece bulutları görebilirim.',
    category: 'phrase'
  },
  {
    id: 'dishwasher',
    word: 'dishwasher',
    translation: 'bulaşık makinesi',
    example: 'Please put your dirty plates in the dishwasher after dinner. - Lütfen akşam yemeğinden sonra kirli tabaklarını bulaşık makinesine koy.',
    category: 'phrase'
  },
  {
    id: 'insect-repellent',
    word: 'insect repellent',
    translation: 'böcek savar',
    example: 'Don\'t forget to use insect repellent to keep the mosquitoes away. - Sivrisinekleri uzak tutmak için böcek savar kullanmayı unutma.',
    category: 'phrase'
  },
  {
    id: 'insights',
    word: 'insights',
    translation: 'iç yüzünü anlamak',
    example: 'The documentary provided deep insights into the lives of wild animals. - Belgesel vahşi hayvanların yaşamlarının iç yüzünü anlamak için derin bilgiler sundu.',
    category: 'phrase'
  },
  {
    id: 'approach-to',
    word: 'approach to',
    translation: 'yaklaşım',
    example: 'We need to find a new approach to solving this math problem. - Bu matematik problemini çözmek için yeni bir yaklaşım bulmalıyız.',
    category: 'phrase'
  },
  {
    id: 'vaccinations',
    word: 'vaccinations',
    translation: 'aşı',
    example: 'Make sure you get all your travel vaccinations before visiting the country. - Ülkeyi ziyaret etmeden önce tüm seyahat aşılarınızı yaptırdığınızdan emin olun.',
    category: 'phrase'
  },
  {
    id: 'plantain',
    word: 'plantain',
    translation: 'muz türü',
    example: 'She fried some plantain to serve with the main dish. - Ana yemekle servis etmek için biraz muz türü (plantain) kızarttı.',
    category: 'phrase'
  },
  {
    id: 'contribute',
    word: 'contribute',
    translation: 'katkıda bulunmak',
    example: 'Everyone should contribute ideas during the brainstorming session. - Beyin fırtınası oturumu sırasında herkes fikirlere katkıda bulunmalıdır.',
    category: 'phrase'
  },
  {
    id: 'herbs',
    word: 'herbs',
    translation: 'bitki',
    example: 'Adding fresh herbs like basil and mint makes the soup taste better. - Fesleğen ve nane gibi taze bitkiler eklemek çorbanın tadını güzelleştirir.',
    category: 'phrase'
  },
  {
    id: 'mention',
    word: 'mention',
    translation: 'bahsetmek',
    example: 'Did he mention what time the meeting is going to start? - Toplantının saat kaçta başlayacağından bahsetti mi?',
    category: 'phrase'
  },
  {
    id: 'in-advance',
    word: 'in advance',
    translation: 'önceden',
    example: 'You need to book your train tickets weeks in advance. - Tren biletlerinizi haftalar önceden ayırtmanız gerekiyor.',
    category: 'phrase'
  },
  {
    id: 'steamed-rice',
    word: 'steamed rice',
    translation: 'buharda pilav',
    example: 'I ordered a bowl of steamed rice to go with the spicy chicken. - Baharatlı tavuğun yanına bir kase buharda pilav sipariş ettim.',
    category: 'phrase'
  },
  {
    id: 'randomly',
    word: 'randomly',
    translation: 'rastgele',
    example: 'The teacher randomly picked a student to answer the question. - Öğretmen soruyu cevaplaması için rastgele bir öğrenci seçti.',
    category: 'phrase'
  },
  {
    id: 'explore',
    word: 'explore',
    translation: 'keşfetmek',
    example: 'We spent the whole afternoon trying to explore the historical ruins. - Bütün öğleden sonrayı tarihi kalıntıları keşfetmeye çalışarak geçirdik.',
    category: 'phrase'
  },
  {
    id: 'bride',
    word: 'bride',
    translation: 'gelin',
    example: 'The bride looked stunning in her elegant white dress. - Gelin zarif beyaz elbisesi içinde çarpıcı görünüyordu.',
    category: 'phrase'
  },
  {
    id: 'groom',
    word: 'groom',
    translation: 'damat',
    example: 'The groom waited nervously at the altar for the ceremony to begin. - Damat törenin başlaması için mihrapta gergin bir şekilde bekledi.',
    category: 'phrase'
  },
  {
    id: 'moving-house',
    word: 'moving house',
    translation: 'ev taşımak',
    example: 'We are moving house next week, so we are busy packing boxes. - Haftaya ev taşıyoruz, bu yüzden kutuları paketlemekle meşgulüz.',
    category: 'phrase'
  },
  {
    id: 'move-out-of',
    word: 'move out of',
    translation: 'bir yerden çıkmak',
    example: 'He decided to move out of his parents\' house and live on his own. - Ailesinin evinden çıkmaya ve kendi başına yaşamaya karar verdi.',
    category: 'phrase'
  },
  {
    id: 'terrace',
    word: 'terrace',
    translation: 'teras',
    example: 'We sat on the terrace enjoying the beautiful sunset. - Güzel gün batımının tadını çıkararak terasta oturduk.',
    category: 'phrase'
  },
  {
    id: 'renting',
    word: 'renting',
    translation: 'kiralamak',
    example: 'Renting an apartment in the city center is becoming very expensive. - Şehir merkezinde bir daire kiralamak çok pahalı hale geliyor.',
    category: 'phrase'
  },
  {
    id: 'block',
    word: 'block',
    translation: 'apartman',
    example: 'They live in the modern block of apartments at the end of the street. - Sokağın sonundaki modern apartman bloğunda yaşıyorlar.',
    category: 'phrase'
  },
  {
    id: 'neighbourhood',
    word: 'neighbourhood',
    translation: 'mahalle',
    example: 'We live in a quiet and friendly neighbourhood. - Sessiz ve samimi bir mahallede yaşıyoruz.',
    category: 'phrase'
  },
  {
    id: 'ground-floor',
    word: 'ground floor',
    translation: 'zemin kat',
    example: 'Our office is located on the ground floor of the building. - Ofisimiz binanın zemin katında yer almaktadır.',
    category: 'phrase'
  },
  {
    id: 'landing',
    word: 'landing',
    translation: 'sahanlık',
    example: 'She waited for him on the landing at the top of the stairs. - Merdivenlerin başındaki sahanlıkta onu bekledi.',
    category: 'phrase'
  },
  {
    id: 'argue-with',
    word: 'argue with',
    translation: 'tartışmak',
    example: 'I don\'t want to argue with you over such a small mistake. - Böyle küçük bir hata yüzünden seninle tartışmak istemiyorum.',
    category: 'phrase'
  },
  {
    id: 'cope-with',
    word: 'cope with',
    translation: 'başa çıkmak',
    example: 'It can be difficult to cope with stress during exam week. - Sınav haftasında stresle başa çıkmak zor olabilir.',
    category: 'phrase'
  },
  {
    id: 'care-about',
    word: 'care about',
    translation: 'umursamak',
    example: 'Good teachers deeply care about the success of their students. - İyi öğretmenler öğrencilerinin başarısını derinden umursarlar.',
    category: 'phrase'
  },
  {
    id: 'worry-about',
    word: 'worry about',
    translation: 'endişelenmek',
    example: 'You shouldn\'t worry about things you cannot control. - Kontrol edemeyeceğin şeyler için endişelenmemelisin.',
    category: 'phrase'
  },
  {
    id: 'complain-about',
    word: 'complain about',
    translation: 'şikayet etmek',
    example: 'Customers often complain about the slow service in this restaurant. - Müşteriler genellikle bu restorandaki yavaş servisten şikayet ederler.',
    category: 'phrase'
  },
  {
    id: 'rely-on',
    word: 'rely on',
    translation: 'güvenmek',
    example: 'You can always rely on me if you need any support. - Herhangi bir desteğe ihtiyacın olursa her zaman bana güvenebilirsin.',
    category: 'phrase'
  },
  {
    id: 'depend-on',
    word: 'depend on',
    translation: 'bağlı olmak',
    example: 'The success of the project will depend on how hard we work. - Projenin başarısı ne kadar sıkı çalıştığımıza bağlı olacak.',
    category: 'phrase'
  },
  {
    id: 'apologise-for',
    word: 'apologise for',
    translation: 'özür dilemek',
    example: 'I must apologise for arriving late to the dinner. - Akşam yemeğine geç geldiğim için özür dilemeliyim.',
    category: 'phrase'
  },
  {
    id: 'believe-in',
    word: 'believe in',
    translation: 'inanmak',
    example: 'If you believe in yourself, you can achieve anything. - Kendine inanırsan, her şeyi başarabilirsin.',
    category: 'phrase'
  },
  {
    id: 'succeed-in',
    word: 'succeed in',
    translation: 'başarmak',
    example: 'She worked hard to succeed in passing the advanced English exam. - İleri düzey İngilizce sınavını geçmeyi başarmak için çok çalıştı.',
    category: 'phrase'
  },
  {
    id: 'belong-to',
    word: 'belong to',
    translation: 'ait olmak',
    example: 'Does this black umbrella belong to anyone in the office? - Bu siyah şemsiye ofisteki herhangi birine ait mi?',
    category: 'phrase'
  },
  {
    id: 'apologise-to',
    word: 'apologise to',
    translation: 'özür dilemek',
    example: 'You should apologise to your sister for breaking her toy. - Oyuncağını kırdığın için kız kardeşinden özür dilemelisin.',
    category: 'phrase'
  },
  {
    id: 'complain-to',
    word: 'complain to',
    translation: 'şikayet etmek',
    example: 'If the hotel room is dirty, you should complain to the manager. - Eğer otel odası kirliyse, yöneticiye şikayet etmelisin.',
    category: 'phrase'
  },
  {
    id: 'outskirts',
    word: 'outskirts',
    translation: 'şehrin etekleri',
    example: 'They bought a quiet house on the outskirts of the city. - Şehrin eteklerinde sessiz bir ev aldılar.',
    category: 'phrase'
  },
  {
    id: 'entire',
    word: 'entire',
    translation: 'tamamı',
    example: 'I was so hungry that I ate the entire pizza by myself. - O kadar açtım ki pizzanın tamamını tek başıma yedim.',
    category: 'phrase'
  },
  {
    id: 'accommodation',
    word: 'accommodation',
    translation: 'konaklama',
    example: 'We need to find cheap accommodation for our trip to London. - Londra seyahatimiz için ucuz bir konaklama bulmalıyız.',
    category: 'phrase'
  },
  {
    id: 'range',
    word: 'range',
    translation: 'aralık',
    example: 'The store offers a wide range of organic products. - Mağaza geniş bir organik ürün aralığı sunuyor.',
    category: 'phrase'
  },
  {
    id: 'hosts',
    word: 'hosts',
    translation: 'ev sahipleri',
    example: 'Our hosts were very welcoming and made us feel at home. - Ev sahiplerimiz çok misafirperverdi ve bizi evimizde hissettirdi.',
    category: 'phrase'
  },
  {
    id: 'nearby',
    word: 'nearby',
    translation: 'yakınlarda',
    example: 'There is a great cafe nearby where we can grab a coffee. - Yakınlarda kahve alabileceğimiz harika bir kafe var.',
    category: 'phrase'
  },
  {
    id: 'theft',
    word: 'theft',
    translation: 'hırsızlık',
    example: 'He reported the theft of his bicycle to the local police. - Bisikletinin hırsızlığını yerel polise bildirdi.',
    category: 'phrase'
  },
  {
    id: 'create',
    word: 'create',
    translation: 'yaratmak',
    example: 'The artist used recycled materials to create a beautiful sculpture. - Sanatçı güzel bir heykel yaratmak için geri dönüştürülmüş malzemeler kullandı.',
    category: 'phrase'
  },
  {
    id: 'hold-a-meeting',
    word: 'hold a meeting',
    translation: 'toplantı yapmak',
    example: 'We will hold a meeting tomorrow to discuss the new project. - Yeni projeyi tartışmak için yarın toplantı yapacağız.',
    category: 'phrase'
  },
  {
    id: 'subscriber',
    word: 'subscriber',
    translation: 'abone',
    example: 'He is a loyal subscriber to several tech magazines. - O, birkaç teknoloji dergisine sadık bir abonedir.',
    category: 'phrase'
  },
  {
    id: 'series',
    word: 'series',
    translation: 'dizi',
    example: 'I spent the whole weekend watching a new drama series on TV. - Bütün hafta sonunu televizyonda yeni bir drama dizisi izleyerek geçirdim.',
    category: 'phrase'
  },
  {
    id: 'episode',
    word: 'episode',
    translation: 'bölüm',
    example: 'The final episode of the show was full of surprising twists. - Şovun son bölümü şaşırtıcı olaylarla doluydu.',
    category: 'phrase'
  },
  {
    id: 'deliver',
    word: 'deliver',
    translation: 'teslimat',
    example: 'The courier will deliver the package to your house tomorrow morning. - Kurye paketi yarın sabah evinize teslim edecek.',
    category: 'phrase'
  },
  {
    id: 'post-on',
    word: 'post on',
    translation: 'gönderi paylaşmak',
    example: 'Don\'t forget to post on social media to announce the event. - Etkinliği duyurmak için sosyal medyada gönderi paylaşmayı unutma.',
    category: 'phrase'
  },
  {
    id: 'advise',
    word: 'advise',
    translation: 'tavsiye etmek',
    example: 'I strongly advise you to study hard before the final exams. - Final sınavlarından önce sıkı çalışmanı şiddetle tavsiye ederim.',
    category: 'phrase'
  },
  {
    id: 'were-offering',
    word: 'were offering',
    translation: 'teklif yapmak',
    example: 'They were offering a massive discount on all electronics yesterday. - Dün tüm elektronik ürünlerde büyük bir teklif yapıyorlardı.',
    category: 'phrase'
  },
  {
    id: 'warn',
    word: 'warn',
    translation: 'uyarmak',
    example: 'I must warn you that the roads are very icy today. - Bugün yolların çok buzlu olduğu konusunda seni uyarmalıyım.',
    category: 'phrase'
  },
  {
    id: 'promise',
    word: 'promise',
    translation: 'söz vermek',
    example: 'I promise to call you as soon as I arrive at the station. - İstasyona varır varmaz seni arayacağıma söz veriyorum.',
    category: 'phrase'
  },
  {
    id: 'recommend',
    word: 'recommend',
    translation: 'tavsiye',
    example: 'Can you recommend a good restaurant for our anniversary dinner? - Yıl dönümü yemeğimiz için iyi bir restoran tavsiye edebilir misin?',
    category: 'phrase'
  },
  {
    id: 'threaten',
    word: 'threaten',
    translation: 'tehdit etmek',
    example: 'The storm began to threaten the small coastal village. - Fırtına küçük sahil köyünü tehdit etmeye başladı.',
    category: 'phrase'
  },
  {
    id: 'offer',
    word: 'offer',
    translation: 'teklif etmek',
    example: 'They decided to offer him the job because of his excellent skills. - Mükemmel becerilerinden dolayı ona işi teklif etmeye karar verdiler.',
    category: 'phrase'
  },
  {
    id: 'suggest',
    word: 'suggest',
    translation: 'önermek',
    example: 'I suggest that we leave early to avoid the heavy traffic. - Yoğun trafikten kaçınmak için erken çıkmamızı öneriyorum.',
    category: 'phrase'
  },
  {
    id: 'attend',
    word: 'attend',
    translation: 'katılmak',
    example: 'All students must attend the lecture on Monday morning. - Tüm öğrenciler pazartesi sabahı derse katılmalıdır.',
    category: 'phrase'
  },
  {
    id: 'basis',
    word: 'basis',
    translation: 'temel',
    example: 'They meet on a regular basis to practice their English speaking. - İngilizce konuşma pratikleri yapmak için düzenli bir temel üzerinde buluşuyorlar.',
    category: 'phrase'
  },
  {
    id: 'come-across',
    word: 'come across',
    translation: 'karşılaşmak',
    example: 'I happened to come across an old friend while walking in the park. - Parkta yürürken tesadüfen eski bir arkadaşla karşılaştım.',
    category: 'phrase'
  },
  {
    id: 'genuine',
    word: 'genuine',
    translation: 'gerçek',
    example: 'Her smile showed that she had a genuine interest in the topic. - Gülümsemesi konuya karşı gerçek bir ilgisi olduğunu gösterdi.',
    category: 'phrase'
  },
  {
    id: 'kept',
    word: 'kept',
    translation: 'tutmak, sürmek',
    example: 'He kept the secret to himself for many years. - Sırrını uzun yıllar kendine sakladı (tuttu).',
    category: 'phrase'
  },
  {
    id: 'submitted',
    word: 'submitted',
    translation: 'teslim etmek',
    example: 'All applications must be submitted by the end of the week. - Tüm başvurular hafta sonuna kadar teslim edilmelidir.',
    category: 'phrase'
  },
  {
    id: 'setting-up',
    word: 'setting up',
    translation: 'kurmak',
    example: 'They are setting up the stage for tonight\'s concert. - Bu geceki konser için sahneyi kuruyorlar.',
    category: 'phrase'
  },
  {
    id: 'get-through',
    word: 'get through',
    translation: 'başarmak',
    example: 'It was a tough time, but we managed to get through it together. - Zor bir dönemdi, ama birlikte atlatmayı başardık.',
    category: 'phrase'
  },
  {
    id: 'according-to',
    word: 'according to',
    translation: 'göre',
    example: 'According to the weather forecast, it will rain tomorrow. - Hava tahminlerine göre, yarın yağmur yağacak.',
    category: 'phrase'
  },
  {
    id: 'wished',
    word: 'wished',
    translation: 'dilemek',
    example: 'I wished I had studied harder for the math exam. - Keşke matematik sınavına daha çok çalışsaydım diye diledim.',
    category: 'phrase'
  },
  {
    id: 'enquiries',
    word: 'enquiries',
    translation: 'soruşturma',
    example: 'For any further enquiries, please contact the main office. - Daha fazla soruşturma için lütfen ana ofisle iletişime geçin.',
    category: 'phrase'
  },
  {
    id: 'pretending',
    word: 'pretending',
    translation: 'mış gibi yapmak',
    example: 'The kids were pretending to be brave explorers in the jungle. - Çocuklar ormanda cesur kaşiflermiş gibi yapıyorlardı.',
    category: 'phrase'
  },
  {
    id: 'believe-to',
    word: 'believe to',
    translation: 'inanmak',
    example: 'This painting is widely believe to be the work of a famous artist. - Bu tablonun yaygın olarak ünlü bir sanatçının eseri olduğuna inanılır.',
    category: 'phrase'
  },
  {
    id: 'disappointed',
    word: 'disappointed',
    translation: 'hayal kırıklığına uğramış',
    example: 'I was very disappointed when my favorite team lost the match. - Favori takımım maçı kaybettiğinde çok hayal kırıklığına uğradım.',
    category: 'phrase'
  },
  {
    id: 'disappointing',
    word: 'disappointing',
    translation: 'hayal kırıklığı yaratan',
    example: 'The ending of the movie was quite disappointing. - Filmin sonu oldukça hayal kırıklığı yaratıcıydı.',
    category: 'phrase'
  },
  {
    id: 'amused',
    word: 'amused',
    translation: 'eğlenmiş',
    example: 'The children were highly amused by the clown\'s funny tricks. - Çocuklar palyaçonun komik numaralarıyla çok eğlenmişlerdi.',
    category: 'phrase'
  },
  {
    id: 'amusing',
    word: 'amusing',
    translation: 'eğlenceli',
    example: 'He told us an amusing story about his travel adventures. - Bize seyahat maceraları hakkında eğlenceli bir hikaye anlattı.',
    category: 'phrase'
  },
  {
    id: 'depressed',
    word: 'depressed',
    translation: 'depresif',
    example: 'She felt depressed after failing the important driving test. - Önemli direksiyon sınavında başarısız olduktan sonra depresif hissetti.',
    category: 'phrase'
  },
  {
    id: 'depressing',
    word: 'depressing',
    translation: 'üzücü',
    example: 'The constant rain all week was really depressing. - Bütün hafta süren sürekli yağmur gerçekten üzücüydü.',
    category: 'phrase'
  },
  {
    id: 'character',
    word: 'character',
    translation: 'karakter',
    example: 'He is the main character in the newly released novel. - Yeni yayımlanan romanın ana karakteri o.',
    category: 'phrase'
  },
  {
    id: 'event',
    word: 'event',
    translation: 'etkinlik',
    example: 'The annual sports event attracts athletes from all over the country. - Yıllık spor etkinliği ülkenin dört bir yanından sporcuları çeker.',
    category: 'phrase'
  },
  {
    id: 'gulf',
    word: 'gulf',
    translation: 'körfez',
    example: 'The ship sailed smoothly across the calm waters of the gulf. - Gemi körfezin sakin sularında pürüzsüzce ilerledi.',
    category: 'phrase'
  },
  {
    id: 'profits',
    word: 'profits',
    translation: 'kâr',
    example: 'The company saw a huge increase in its profits this year. - Şirket bu yıl kârında büyük bir artış gördü.',
    category: 'phrase'
  },
  {
    id: 'charity',
    word: 'charity',
    translation: 'hayır kurumu',
    example: 'She donated all of her old clothes to a local charity. - Tüm eski kıyafetlerini yerel bir hayır kurumuna bağışladı.',
    category: 'phrase'
  },
  {
    id: 'beauty',
    word: 'beauty',
    translation: 'güzellik',
    example: 'The natural beauty of the national park is truly breathtaking. - Milli parkın doğal güzelliği gerçekten nefes kesicidir.',
    category: 'phrase'
  },
  {
    id: 'celebration',
    word: 'celebration',
    translation: 'kutlama',
    example: 'We are having a big celebration for his retirement next week. - Haftaya onun emekliliği için büyük bir kutlama yapıyoruz.',
    category: 'phrase'
  },
  {
    id: 'celebrate',
    word: 'celebrate',
    translation: 'kutlamak',
    example: 'How do you usually celebrate your birthday with your friends? - Doğum gününü arkadaşlarınla genelde nasıl kutlarsın?',
    category: 'phrase'
  },
  {
    id: 'charitable',
    word: 'charitable',
    translation: 'hayırsever',
    example: 'They do a lot of charitable work to help the homeless. - Evsizlere yardım etmek için çok sayıda hayırsever iş yaparlar.',
    category: 'phrase'
  },
  {
    id: 'creative',
    word: 'creative',
    translation: 'yaratıcı',
    example: 'She is a creative designer who always comes up with unique ideas. - O, her zaman eşsiz fikirler bulan yaratıcı bir tasarımcıdır.',
    category: 'phrase'
  },
  {
    id: 'creativity',
    word: 'creativity',
    translation: 'yaratıcılık',
    example: 'Writing a good story requires a lot of imagination and creativity. - İyi bir hikaye yazmak çok fazla hayal gücü ve yaratıcılık gerektirir.',
    category: 'phrase'
  },
  {
    id: 'cultural',
    word: 'cultural',
    translation: 'kültürel',
    example: 'The city is famous for its historical and cultural heritage. - Şehir, tarihi ve kültürel mirasıyla ünlüdür.',
    category: 'phrase'
  },
  {
    id: 'development',
    word: 'development',
    translation: 'gelişim',
    example: 'Healthy food is essential for the physical development of children. - Sağlıklı yiyecekler çocukların fiziksel gelişimi için gereklidir.',
    category: 'phrase'
  },
  {
    id: 'musical',
    word: 'musical',
    translation: 'müzikal',
    example: 'He showed a great musical talent from a very young age. - Çok genç yaşlardan itibaren harika bir müzikal yetenek gösterdi.',
    category: 'phrase'
  },
  {
    id: 'musician',
    word: 'musician',
    translation: 'müzisyen',
    example: 'The famous musician performed perfectly in front of thousands of fans. - Ünlü müzisyen binlerce hayranının önünde mükemmel bir performans sergiledi.',
    category: 'phrase'
  },
  {
    id: 'organised',
    word: 'organised',
    translation: 'organize',
    example: 'She is very organised and always keeps her desk perfectly clean. - O çok organizedir ve masasını her zaman mükemmel temiz tutar.',
    category: 'phrase'
  },
  {
    id: 'organiser',
    word: 'organiser',
    translation: 'organizatör',
    example: 'The event organiser made sure everything went smoothly. - Etkinlik organizatörü her şeyin sorunsuz gitmesini sağladı.',
    category: 'phrase'
  },
  {
    id: 'organize',
    word: 'organize',
    translation: 'organize etmek',
    example: 'We need to organize a surprise party for her birthday. - Onun doğum günü için sürpriz bir parti organize etmeliyiz.',
    category: 'phrase'
  },
  {
    id: 'performer',
    word: 'performer',
    translation: 'sanatçı',
    example: 'The street performer amazed the crowd with his magic tricks. - Sokak sanatçısı sihir numaralarıyla kalabalığı şaşkına çevirdi.',
    category: 'phrase'
  },
  {
    id: 'perform',
    word: 'perform',
    translation: 'performans sergilemek',
    example: 'The choir will perform classical songs at the winter concert. - Koro kış konserinde klasik şarkılar performans sergileyecek.',
    category: 'phrase'
  },
  {
    id: 'wait-a-second',
    word: 'wait a second',
    translation: 'bir saniye bekle',
    example: 'Please wait a second, I need to find my keys. - Lütfen bir saniye bekle, anahtarlarımı bulmam lazım.',
    category: 'phrase'
  },
  {
    id: 'one-moment-please',
    word: 'one moment please',
    translation: 'bir an lütfen',
    example: 'One moment please, I will connect you to the manager. - Bir an lütfen, sizi yöneticiye bağlayacağım.',
    category: 'phrase'
  },
  {
    id: 'let-me-check-for-you',
    word: 'let me check for you',
    translation: 'sizin için kontrol edeyim',
    example: 'Let me check for you if we have that shirt in your size. - O gömleğin bedeninize uygun olanı var mı, sizin için kontrol edeyim.',
    category: 'phrase'
  },
  {
    id: 'misses',
    word: 'misses',
    translation: 'ıskalamak',
    example: 'If he misses the train, he will be late for the meeting. - Treni ıskalarsa, toplantıya geç kalacak.',
    category: 'phrase'
  },
  {
    id: 'referee',
    word: 'referee',
    translation: 'hakem',
    example: 'The referee blew the whistle to start the football match. - Hakem futbol maçını başlatmak için düdük çaldı.',
    category: 'phrase'
  },
  {
    id: 'afraid-of',
    word: 'afraid of',
    translation: 'korkmak',
    example: 'My little brother is very afraid of the dark. - Küçük kardeşim karanlıktan çok korkar.',
    category: 'phrase'
  },
  {
    id: 'scared-of',
    word: 'scared of',
    translation: 'korkmuş',
    example: 'She was too scared of heights to look down from the balcony. - Balkondan aşağı bakamayacak kadar yükseklikten korkmuştu.',
    category: 'phrase'
  },
  {
    id: 'essential-for',
    word: 'essential for',
    translation: 'gerekli',
    example: 'Water is absolutely essential for the survival of all living things. - Su, tüm canlıların hayatta kalması için kesinlikle gereklidir.',
    category: 'phrase'
  },
  {
    id: 'nervous',
    word: 'nervous',
    translation: 'gergin',
    example: 'I always feel nervous before speaking in front of a large crowd. - Büyük bir kalabalığın önünde konuşmadan önce her zaman gergin hissederim.',
    category: 'phrase'
  },
  {
    id: 'tired-of',
    word: 'tired of',
    translation: 'bıkmış',
    example: 'I am so tired of doing the same boring tasks every single day. - Her gün aynı sıkıcı görevleri yapmaktan çok bıktım.',
    category: 'phrase'
  },
  {
    id: 'right-for',
    word: 'right for',
    translation: 'için doğru',
    example: 'You need to decide if this career path is really right for you. - Bu kariyer yolunun senin için gerçekten doğru olup olmadığına karar vermelisin.',
    category: 'phrase'
  },
  {
    id: 'similar-to',
    word: 'similar to',
    translation: 'benzer',
    example: 'Your new phone looks very similar to the one I bought last year. - Yeni telefonun geçen yıl aldığım telefona çok benzer görünüyor.',
    category: 'phrase'
  },
  {
    id: 'popular-with',
    word: 'popular with',
    translation: 'ile popüler',
    example: 'The new teacher is extremely popular with all the students. - Yeni öğretmen tüm öğrencilerle son derece popüler.',
    category: 'phrase'
  },
  {
    id: 'perfect-to',
    word: 'perfect to',
    translation: 'için mükemmel',
    example: 'This sunny weather is perfect to go for a long walk in the park. - Bu güneşli hava parkta uzun bir yürüyüşe çıkmak için mükemmel.',
    category: 'phrase'
  },
  {
    id: 'definitely',
    word: 'definitely',
    translation: 'kesinlikle',
    example: 'I will definitely call you when I get home tonight. - Bu gece eve vardığımda seni kesinlikle arayacağım.',
    category: 'phrase'
  },
  {
    id: 'speaking-of',
    word: 'speaking of',
    translation: 'demişken',
    example: 'Speaking of movies, did you watch the new sci-fi film yet? - Filmlerden demişken, yeni bilim kurgu filmini izledin mi?',
    category: 'phrase'
  },
  {
    id: 'that-reminds-me',
    word: 'that reminds me',
    translation: 'o bana şunu hatırlattı',
    example: 'That reminds me, I need to buy some milk on the way back. - O bana şunu hatırlattı, dönüş yolunda biraz süt almam lazım.',
    category: 'phrase'
  },
  {
    id: 'you-ve-got-nothing',
    word: 'you\'ve got nothing',
    translation: 'hiçbir şeyin yok',
    example: 'Feel free to reach out if you need any help with your project. - Projenle ilgili yardıma ihtiyacın olursa bana ulaşmaktan çekinme.',
    category: 'phrase'
  },
  {
    id: 'by-the-way',
    word: 'by the way',
    translation: 'bu arada',
    example: 'By the way, what time does the concert start tonight? - Bu arada, bu gece konser saat kaçta başlıyor?',
    category: 'phrase'
  },
  {
    id: 'i-was-saying',
    word: 'I was saying',
    translation: 'diyordum ki',
    example: 'As I was saying before we were interrupted, the project is almost done. - Bölünmeden önce diyordum ki, proje neredeyse bitti.',
    category: 'phrase'
  },
  {
    id: 'i-m-still-worried',
    word: 'I\'m still worried',
    translation: 'hala endişeliyim',
    example: 'By the way, did you finish reading that book I lent you? - Bu arada, sana ödünç verdiğim o kitabı okumayı bitirdin mi?',
    category: 'phrase'
  },
  {
    id: 'identify',
    word: 'identify',
    translation: 'belirlemek',
    example: 'It is important to identify the problem before trying to solve it. - Çözmeye çalışmadan önce sorunu belirlemek önemlidir.',
    category: 'phrase'
  },
  {
    id: 'disappear',
    word: 'disappear',
    translation: 'kaybolmak',
    example: 'The magician made the coin disappear in front of our eyes. - Sihirbaz gözlerimizin önünde madeni parayı kaybolmak (yok etmek) suretiyle büyüledi.',
    category: 'phrase'
  },
  {
    id: 'value',
    word: 'value',
    translation: 'değer',
    example: 'This antique vase has a very high historical value. - Bu antika vazonun çok yüksek tarihi bir değeri var.',
    category: 'phrase'
  },
  {
    id: 'hurt',
    word: 'hurt',
    translation: 'incitmek',
    example: 'Be careful with that knife, you might hurt yourself. - O bıçakla dikkatli ol, kendini incitirsin.',
    category: 'phrase'
  },
  {
    id: 'adopted',
    word: 'adopted',
    translation: 'evlat edinmek',
    example: 'They decided to adopt a beautiful rescue dog from the shelter. - Barınaktan güzel bir kurtarma köpeği evlat edinmeye karar verdiler.',
    category: 'phrase'
  },
  {
    id: 'honour',
    word: 'honour',
    translation: 'onur',
    example: 'It is a great honour to receive this prestigious award today. - Bugün bu prestijli ödülü almak büyük bir onur.',
    category: 'phrase'
  },
  {
    id: 'retirement',
    word: 'retirement',
    translation: 'emeklilik',
    example: 'He is planning to travel the world after his retirement next year. - Gelecek yılki emekliliğinden sonra dünyayı gezmeyi planlıyor.',
    category: 'phrase'
  },
  {
    id: 'thick',
    word: 'thick',
    translation: 'kalın',
    example: 'You should wear a thick coat because it is freezing outside. - Dışarısı dondurucu soğuk olduğu için kalın bir kaban giymelisin.',
    category: 'phrase'
  },
  {
    id: 'constantly',
    word: 'constantly',
    translation: 'sürekli',
    example: 'The baby was crying constantly throughout the entire flight. - Bebek tüm uçuş boyunca sürekli ağlıyordu.',
    category: 'phrase'
  },
  {
    id: 'reached',
    word: 'reached',
    translation: 'ulaşmak',
    example: 'We finally reached the top of the mountain after hours of climbing. - Saatler süren tırmanışın ardından nihayet dağın zirvesine ulaştık.',
    category: 'phrase'
  },
  {
    id: 'negotiate',
    word: 'negotiate',
    translation: 'müzakere',
    example: 'They had to negotiate a better deal with the suppliers. - Tedarikçilerle daha iyi bir anlaşmayı müzakere etmek zorundaydılar.',
    category: 'phrase'
  },
  {
    id: 'pressure',
    word: 'pressure',
    translation: 'baskı',
    example: 'She performs very well even under extreme pressure at work. - İş yerinde aşırı baskı altındayken bile çok iyi performans sergiler.',
    category: 'phrase'
  },
  {
    id: 'ambitious',
    word: 'ambitious',
    translation: 'hırslı',
    example: 'He is an ambitious young lawyer who wants to become a partner soon. - Yakında ortak olmak isteyen hırslı genç bir avukattır.',
    category: 'phrase'
  },
  {
    id: 'anxious',
    word: 'anxious',
    translation: 'endişeli',
    example: 'I always feel anxious waiting for my exam results to be announced. - Sınav sonuçlarımın açıklanmasını beklerken her zaman endişeli hissederim.',
    category: 'phrase'
  },
  {
    id: 'misjudged',
    word: 'misjudged',
    translation: 'yanlış yargılama',
    example: 'I completely misjudged his character; he is actually a very nice guy. - Karakterini tamamen yanlış yargılamışım; aslında çok iyi bir adam.',
    category: 'phrase'
  },
  {
    id: 'smoother',
    word: 'smoother',
    translation: 'pürüzsüz',
    example: 'Using this special lotion makes your skin feel much smoother. - Bu özel losyonu kullanmak cildinizin çok daha pürüzsüz hissetmesini sağlar.',
    category: 'phrase'
  },
  {
    id: 'nowadays',
    word: 'nowadays',
    translation: 'bugünlerde',
    example: 'Nowadays, teenagers spend too much time on their mobile phones. - Bugünlerde gençler cep telefonlarında çok fazla zaman harcıyorlar.',
    category: 'phrase'
  },
  {
    id: 'presence',
    word: 'presence',
    translation: 'var olmak',
    example: 'Her calm presence in the room helped everyone relax. - Odadaki sakin varlığı herkesin rahatlamasına yardımcı oldu.',
    category: 'phrase'
  },
  {
    id: 'avoid',
    word: 'avoid',
    translation: 'kaçınmak',
    example: 'You should try to avoid eating too much sugar for a healthy diet. - Sağlıklı bir diyet için çok fazla şeker yemekten kaçınmaya çalışmalısın.',
    category: 'phrase'
  },
  {
    id: 'adapted',
    word: 'adapted',
    translation: 'uyarlanmış',
    example: 'The popular novel was successfully adapted into an award-winning movie. - Popüler roman başarılı bir şekilde ödüllü bir filme uyarlandı.',
    category: 'phrase'
  },
  {
    id: 'award',
    word: 'award',
    translation: 'ödül',
    example: 'She won the first prize award for her excellent science project. - Mükemmel bilim projesi için birincilik ödülünü kazandı.',
    category: 'phrase'
  },
  {
    id: 'mess-with',
    word: 'mess with',
    translation: 'bulaşmak',
    example: 'You shouldn\'t mess with the wiring if you don\'t know what you are doing. - Ne yaptığını bilmiyorsan kablolarla bulaşmamalısın.',
    category: 'phrase'
  },
  {
    id: 'miss',
    word: 'miss',
    translation: 'özlemek',
    example: 'I really miss the traditional food my grandmother used to make. - Büyükannemin eskiden yaptığı geleneksel yemekleri gerçekten özlüyorum.',
    category: 'phrase'
  },
  {
    id: 'thin',
    word: 'thin',
    translation: 'ince',
    example: 'The ice on the lake is too thin to walk on safely. - Göldeki buz güvenle yürümek için çok ince.',
    category: 'phrase'
  },
  {
    id: 'closed',
    word: 'closed',
    translation: 'kapalı',
    example: 'The library is closed on Sundays and public holidays. - Kütüphane pazar günleri ve resmi tatillerde kapalıdır.',
    category: 'phrase'
  },
  {
    id: 'shelves',
    word: 'shelves',
    translation: 'raf',
    example: 'We need to put up some new shelves to hold all these books. - Tüm bu kitapları koymak için bazı yeni raflar kurmalıyız.',
    category: 'phrase'
  },
  {
    id: 'rebellious',
    word: 'rebellious',
    translation: 'asi',
    example: 'As a teenager, he had a very rebellious attitude towards authority. - Bir genç olarak, otoriteye karşı çok asi bir tutumu vardı.',
    category: 'phrase'
  },
  {
    id: 'relaxed',
    word: 'relaxed',
    translation: 'rahat',
    example: 'He looked completely relaxed while reading a book on the beach. - Sahilde kitap okurken tamamen rahat görünüyordu.',
    category: 'phrase'
  },
  {
    id: 'therefore',
    word: 'therefore',
    translation: 'bu yüzden',
    example: 'He was injured, and therefore he couldn\'t play in the final match. - O yaralıydı ve bu yüzden final maçında oynayamadı.',
    category: 'phrase'
  },
  {
    id: 'provide',
    word: 'provide',
    translation: 'sağlamak',
    example: 'The hotel will provide fresh towels and soap for all guests. - Otel tüm misafirler için taze havlu ve sabun sağlayacaktır.',
    category: 'phrase'
  },
  {
    id: 'deal-with',
    word: 'deal with',
    translation: 'ilgilenmek',
    example: 'She has to deal with angry customers every day at her job. - İşinde her gün kızgın müşterilerle ilgilenmek zorundadır.',
    category: 'phrase'
  },
  {
    id: 'improve',
    word: 'improve',
    translation: 'iyileştirmek',
    example: 'Practicing every day will help you improve your speaking skills. - Her gün pratik yapmak konuşma becerilerini iyileştirmene yardımcı olacaktır.',
    category: 'phrase'
  },

  {
    id: 'v-c-make-a-good-impression',
    word: 'make a good impression',
    translation: 'iyi bir izlenim bırakmak',
    example: 'It is important to dress nicely to make a good impression at the interview. - Görüşmede iyi bir izlenim bırakmak için güzel giyinmek önemlidir.',
    category: 'phrase',
  },
  {
    id: 'v-c-i-m-not-so-sure-about-that',
    word: 'I\'m not so sure about that',
    translation: 'Ondan o kadar emin değilim',
    example: 'The scientist was able to identify the rare bird species. - Bilim insanı nadir kuş türünü belirlemeyi başardı.',
    category: 'phrase',
  },
  {
    id: 'v-c-i-see-where-you-re-coming-from',
    word: 'I see where you\'re coming from',
    translation: 'Ne demek istediğini anlıyorum',
    example: 'I see where you\'re coming from, but I still think we should try the other option. - Ne demek istediğini anlıyorum ama hala diğer seçeneği denememiz gerektiğini düşünüyorum.',
    category: 'phrase',
  },
  {
    id: 'v-c-to-be-turned-into',
    word: 'to be turned into',
    translation: 'dönüştürülmek',
    example: 'The old factory is going to be turned into a modern art museum. - Eski fabrika modern bir sanat müzesine dönüştürülecek.',
    category: 'phrase',
  },
  {
    id: 'v-c-take-it-back',
    word: 'take it back',
    translation: 'geri götürmek, iade etmek',
    example: 'If the shirt doesn\'t fit, you can take it back to the store. - Eğer gömlek uymazsa, onu mağazaya geri götürebilirsin.',
    category: 'phrase',
  },
  {
    id: 'v-c-turn-it-off-and-on-again',
    word: 'turn it off and on again',
    translation: 'kapatıp tekrar açmak',
    example: 'When my computer freezes, I usually just turn it off and on again. - Bilgisayarım donduğunda, genelde sadece kapatıp tekrar açarım.',
    category: 'phrase',
  },
  {
    id: 'v-c-claim-on-your-insurance',
    word: 'claim on your insurance',
    translation: 'sigortadan talep etmek',
    example: 'If your car is stolen, you must make a claim on your insurance. - Eğer arabanız çalınırsa, sigortadan talep etmek zorundasınız.',
    category: 'phrase',
  },
  {
    id: 'v-c-what-have-i-got-to-lose',
    word: 'what have I got to lose',
    translation: 'kaybedecek neyim var ki',
    example: 'I might as well apply for the job; what have I got to lose? - İşe başvursam iyi olur; kaybedecek neyim var ki?',
    category: 'phrase',
  },
  {
    id: 'v-c-broken-down',
    word: 'broken down',
    translation: 'bozulmak (araba, makine vb.)',
    example: 'My car has broken down twice this week on the highway. - Arabam bu hafta otoyolda iki kez bozuldu.',
    category: 'phrase',
  },
  {
    id: 'v-c-spill',
    word: 'spill',
    translation: 'dökmek (sıvı)',
    example: 'Be careful not to spill hot coffee on your clean shirt. - Temiz gömleğine sıcak kahve dökmemeye dikkat et.',
    category: 'phrase',
  },
  {
    id: 'v-c-annoying',
    word: 'annoying',
    translation: 'sinir bozucu',
    example: 'That loud noise coming from the street is really annoying. - Sokaktan gelen o yüksek ses gerçekten sinir bozucu.',
    category: 'phrase',
  },
  {
    id: 'v-c-frozen-screen',
    word: 'frozen screen',
    translation: 'donmuş ekran',
    example: 'I couldn\'t save my document because of the frozen screen. - Donmuş ekran yüzünden belgemi kaydedemedim.',
    category: 'phrase',
  },
  {
    id: 'v-c-you-won-t-believe-what-i-did',
    word: 'you won\'t believe what I did',
    translation: 'ne yaptığıma inanamayacaksın',
    example: 'The old hospital is going to be turned into luxury apartments. - Eski hastane lüks dairelere dönüştürülecek.',
    category: 'phrase',
  },
  {
    id: 'v-c-it-turned-out',
    word: 'it turned out',
    translation: 'ortaya çıktı ki, meğerse',
    example: 'It turned out that we had been completely wrong all along. - Ortaya çıktı ki başından beri tamamen haksızmışız.',
    category: 'phrase',
  },
  {
    id: 'v-c-coincidence',
    word: 'coincidence',
    translation: 'tesadüf',
    example: 'Meeting you here in Paris is such a crazy coincidence! - Seninle burada Paris\'te karşılaşmak ne çılgın bir tesadüf!',
    category: 'phrase',
  },
  {
    id: 'v-c-in-the-end',
    word: 'in the end',
    translation: 'sonunda',
    example: 'We tried our best, but in the end, it wasn\'t enough. - Elimizden gelenin en iyisini yaptık, ama sonunda yeterli olmadı.',
    category: 'phrase',
  },
  {
    id: 'v-c-luckily',
    word: 'luckily',
    translation: 'şans eseri, neyse ki',
    example: 'Luckily, I found my lost wallet under the car seat. - Şans eseri, kayıp cüzdanımı araba koltuğunun altında buldum.',
    category: 'phrase',
  },
  {
    id: 'v-c-straight-away',
    word: 'straight away',
    translation: 'hemen, derhal',
    example: 'If you feel chest pain, you should see a doctor straight away. - Eğer göğüs ağrısı hissederseniz, derhal bir doktora görünmelisiniz.',
    category: 'phrase',
  },
  {
    id: 'v-c-freecycling',
    word: 'freecycling',
    translation: 'bedava eşya takası',
    example: 'Freecycling is a great way to give away things you no longer need. - Bedava eşya takası, artık ihtiyacınız olmayan eşyaları vermenin harika bir yoludur.',
    category: 'phrase',
  },
  {
    id: 'v-c-get-a-refund',
    word: 'get a refund',
    translation: 'para iadesi almak',
    example: 'If the product is damaged, you have the right to get a refund. - Eğer ürün hasarlıysa, para iadesi alma hakkınız vardır.',
    category: 'phrase',
  },
  {
    id: 'v-c-do-you-need-a-hand',
    word: 'do you need a hand',
    translation: 'yardıma ihtiyacın var mı',
    example: 'Those boxes look very heavy, do you need a hand carrying them? - O kutular çok ağır görünüyor, taşırken yardıma ihtiyacın var mı?',
    category: 'phrase',
  },
  {
    id: 'v-c-ask-a-favour-in-return',
    word: 'ask a favour in return',
    translation: 'karşılığında bir iyilik istemek',
    example: 'I will help you, but I might ask a favour in return someday. - Sana yardım edeceğim, ama bir gün karşılığında bir iyilik isteyebilirim.',
    category: 'phrase',
  },
  {
    id: 'v-c-give-someone-a-hand',
    word: 'give someone a hand',
    translation: 'birine yardım etmek',
    example: 'Can you give me a hand with this difficult math homework? - Bu zor matematik ödevinde bana yardım edebilir misin?',
    category: 'phrase',
  },
  {
    id: 'v-c-take-photos',
    word: 'take photos',
    translation: 'fotoğraf çekmek',
    example: 'We stopped at the beautiful bridge to take photos of the sunset. - Gün batımının fotoğraf çekmek için o güzel köprüde durduk.',
    category: 'phrase',
  },
  {
    id: 'v-c-give-someone-a-lift',
    word: 'give someone a lift',
    translation: 'birini arabayla bırakmak',
    example: 'It is raining heavily, let me give you a lift to the station. - Şiddetli yağmur yağıyor, seni arabayla istasyona bırakayım.',
    category: 'phrase',
  },
  {
    id: 'v-c-talk-through',
    word: 'talk through',
    translation: 'enine boyuna konuşmak, tartışmak',
    example: 'Let\'s sit down and talk through the problem before making a decision. - Karar vermeden önce oturup problemi enine boyuna tartışalım.',
    category: 'phrase',
  },
  {
    id: 'v-c-for-example',
    word: 'for example',
    translation: 'örneğin',
    example: 'You can eat many healthy foods, for example, apples and carrots. - Birçok sağlıklı yiyecek yiyebilirsiniz, örneğin, elma ve havuç.',
    category: 'phrase',
  },
  {
    id: 'v-c-spread-my-wings',
    word: 'spread my wings',
    translation: 'kanatlarını açmak, yeni şeyler denemek',
    example: 'Going to university abroad will allow me to spread my wings. - Yurtdışında üniversiteye gitmek kanatlarını açmama (yeni şeyler denememe) olanak tanıyacak.',
    category: 'phrase',
  },
  {
    id: 'v-c-resigned',
    word: 'resigned',
    translation: 'istifa etmek',
    example: 'The manager resigned from his position after the scandal was revealed. - Yönetici skandal ortaya çıktıktan sonra görevinden istifa etti.',
    category: 'phrase',
  },
  {
    id: 'v-c-bored-in-my-job',
    word: 'bored in my job',
    translation: 'işimden sıkıldım',
    example: 'I feel so bored in my job because every day is exactly the same. - İşimden sıkıldım çünkü her gün tamamen aynı.',
    category: 'phrase',
  },
  {
    id: 'v-c-ask-for-recommendations',
    word: 'ask for recommendations',
    translation: 'tavsiye istemek',
    example: 'We should ask for recommendations from the locals for a good restaurant. - İyi bir restoran için yerel halktan tavsiye istemeliyiz.',
    category: 'phrase',
  },
  {
    id: 'v-c-worth-asking',
    word: 'worth asking',
    translation: 'sormaya değer',
    example: 'I don\'t think he will agree, but it is still worth asking. - Kabul edeceğini sanmıyorum ama yine de sormaya değer.',
    category: 'phrase',
  },
  {
    id: 'v-c-huge-diamond',
    word: 'huge diamond',
    translation: 'kocaman elmas',
    example: 'The thief managed to steal the huge diamond from the museum display. - Hırsız müze vitrinindeki kocaman elması çalmayı başardı.',
    category: 'phrase',
  },
  {
    id: 'v-c-ridiculous',
    word: 'ridiculous',
    translation: 'saçma, gülünç',
    example: 'It is absolutely ridiculous to pay that much money for a plain t-shirt. - Düz bir tişört için o kadar para ödemek kesinlikle gülünç.',
    category: 'phrase',
  },
  {
    id: 'v-c-take-presents',
    word: 'take presents',
    translation: 'hediye götürmek',
    example: 'It is polite to take presents when you are invited to someone\'s house. - Birinin evine davet edildiğinde hediye götürmek kibarlıktır.',
    category: 'phrase',
  },
  {
    id: 'v-c-arrive-on-time',
    word: 'arrive on time',
    translation: 'zamanında varmak',
    example: 'Please make sure to arrive on time for your dentist appointment. - Lütfen dişçi randevunuza zamanında varmaya özen gösterin.',
    category: 'phrase',
  },
  {
    id: 'v-c-take-your-shoes-off',
    word: 'take your shoes off',
    translation: 'ayakkabılarını çıkarmak',
    example: 'In our culture, you must take your shoes off before entering the house. - Kültürümüzde, eve girmeden önce ayakkabılarını çıkarmalısın.',
    category: 'phrase',
  },
  {
    id: 'v-c-is-there-anything-we-can-do-to-help',
    word: 'is there anything we can do to help',
    translation: 'yardım edebileceğimiz bir şey var mı',
    example: 'We heard the bad news; is there anything we can do to help? - Kötü haberi duyduk; yardım edebileceğimiz bir şey var mı?',
    category: 'phrase',
  },
  {
    id: 'v-c-is-it-ok-if-i',
    word: 'is it OK if I',
    translation: 'yapsam olur mu / sorun olur mu',
    example: 'Is it OK if I leave the office early today to see my doctor? - Bugün doktorumu görmek için ofisten erken çıksam sorun olur mu?',
    category: 'phrase',
  },
  {
    id: 'v-c-it-s-all-under-control',
    word: 'it\'s all under control',
    translation: 'her şey kontrol altında',
    example: 'We were worried about the exam, but it turned out to be quite easy. - Sınav için endişeliydik ama ortaya çıktı ki oldukça kolaymış.',
    category: 'phrase',
  },
  {
    id: 'v-c-do-you-mind-if-i',
    word: 'do you mind if I',
    translation: 'yapsam sakıncası var mı',
    example: 'Do you mind if I open the window to get some fresh air? - Biraz temiz hava almak için pencereyi açsam sakıncası var mı?',
    category: 'phrase',
  },
  {
    id: 'v-c-not-at-all',
    word: 'not at all',
    translation: 'hiç de bile, rica ederim',
    example: 'Do you mind if I sit here? - Not at all, please go ahead. - Buraya oturmamın sakıncası var mı? - Hiç de bile, lütfen buyrun.',
    category: 'phrase',
  },
  {
    id: 'v-c-it-s-hot-in-here',
    word: 'it\'s hot in here',
    translation: 'burası çok sıcak',
    example: 'Do you mind if I sit next to you during the presentation? - Sunum sırasında yanına otursam sakıncası var mı?',
    category: 'phrase',
  },
  {
    id: 'v-c-i-imagine-you-re',
    word: 'I imagine you\'re',
    translation: 'olduğunu tahmin ediyorum',
    example: 'I imagine you\'re tired after such a long journey. - Böyle uzun bir yolculuktan sonra yorgun olduğunu tahmin ediyorum.',
    category: 'phrase',
  },
  {
    id: 'v-c-that-must-be-exciting',
    word: 'that must be exciting',
    translation: 'bu heyecan verici olmalı',
    example: 'You are traveling to Japan? Wow, that must be exciting! - Japonya\'ya mı seyahat ediyorsun? Vay canına, bu heyecan verici olmalı!',
    category: 'phrase',
  },
  {
    id: 'v-c-action-shots',
    word: 'action shots',
    translation: 'hareketli çekimler',
    example: 'The sports photographer took incredible action shots during the final match. - Spor fotoğrafçısı final maçı sırasında inanılmaz hareketli çekimler yaptı.',
    category: 'phrase',
  },
  {
    id: 'v-c-generally',
    word: 'generally',
    translation: 'genellikle',
    example: 'Generally, it is warmer in the south than in the north. - Genellikle güney, kuzeyden daha sıcaktır.',
    category: 'phrase',
  },
  {
    id: 'v-c-normally',
    word: 'normally',
    translation: 'normalde',
    example: 'I normally wake up at 7 AM, but today I slept until 9 AM. - Normalde sabah 7\'de uyanırım ama bugün 9\'a kadar uyudum.',
    category: 'phrase',
  },
  {
    id: 'v-c-typically',
    word: 'typically',
    translation: 'tipik olarak',
    example: 'Typically, a British breakfast includes eggs, bacon, and beans. - Tipik olarak, bir İngiliz kahvaltısı yumurta, pastırma ve fasulye içerir.',
    category: 'phrase',
  },
  {
    id: 'v-c-as-a-rule',
    word: 'as a rule',
    translation: 'kural olarak, genel olarak',
    example: 'As a rule, you should not eat heavily right before going to bed. - Genel kural olarak, yatmadan hemen önce ağır yememelisiniz.',
    category: 'phrase',
  },
  {
    id: 'v-c-a-couple-of',
    word: 'a couple of',
    translation: 'birkaç',
    example: 'I just need a couple of minutes to finish typing this email. - Bu e-postayı yazmayı bitirmek için birkaç dakikaya ihtiyacım var.',
    category: 'phrase',
  },
  {
    id: 'v-c-sort-of-thing',
    word: 'sort of thing',
    translation: 'o tarz şeyler',
    example: 'I enjoy hiking, camping, and that sort of thing. - Doğa yürüyüşü, kamp ve o tarz şeylerden hoşlanırım.',
    category: 'phrase',
  },
  {
    id: 'v-c-kind-of-thing',
    word: 'kind of thing',
    translation: 'bu tarz bir şey',
    example: 'Action movies aren\'t really my kind of thing. - Aksiyon filmleri pek benim tarzım bir şey değil.',
    category: 'phrase',
  },
  {
    id: 'v-c-vague-language',
    word: 'vague language',
    translation: 'belirsiz dil',
    example: 'Politicians often use vague language to avoid answering direct questions. - Politikacılar doğrudan soruları yanıtlamaktan kaçınmak için genellikle belirsiz dil kullanırlar.',
    category: 'phrase',
  },
  {
    id: 'v-c-except-for',
    word: 'except for',
    translation: 'haricinde, -den başka',
    example: 'Everyone attended the meeting except for John, who was sick. - Hasta olan John haricinde herkes toplantıya katıldı.',
    category: 'phrase',
  },
  {
    id: 'v-c-it-s-meant-to-be',
    word: 'it\'s meant to be',
    translation: 'olduğu söyleniyor, olması bekleniyor',
    example: 'That movie is meant to be very good. - O filmin çok iyi olduğu söyleniyor.',
    category: 'phrase',
  },
  {
    id: 'v-c-highly-recommended',
    word: 'highly recommended',
    translation: 'şiddetle tavsiye edilen',
    example: 'This book is highly recommended for anyone interested in history. - Bu kitap tarihle ilgilenen herkes için şiddetle tavsiye edilir.',
    category: 'phrase',
  },
  {
    id: 'v-c-it-s-supposed-to-be',
    word: 'it\'s supposed to be',
    translation: 'olduğu söyleniyor, olması bekleniyor',
    example: 'It\'s supposed to be sunny today. - Bugün havanın güneşli olması bekleniyor.',
    category: 'phrase',
  },
  {
    id: 'v-c-i-m-not-a-big-fan-of',
    word: 'I\'m not a big fan of',
    translation: 'nın büyük bir hayranı değilim, pek sevmem',
    example: 'I\'m not a big fan of spicy food, to be honest. - Dürüst olmak gerekirse, acılı yemeklerin pek hayranı değilim.',
    category: 'phrase',
  },
  {
    id: 'v-c-i-doubt',
    word: 'I doubt',
    translation: 'şüpheliyim',
    example: 'I doubt that they will arrive on time in this heavy traffic. - Bu yoğun trafikte zamanında varacaklarından şüpheliyim.',
    category: 'phrase',
  },
  {
    id: 'v-c-just-a-moment',
    word: 'just a moment',
    translation: 'sadece bir an, bir saniye',
    example: 'Could you wait just a moment while I find your file? - Dosyanızı bulurken sadece bir saniye bekler misiniz?',
    category: 'phrase',
  },
  {
    id: 'v-c-let-me-check',
    word: 'let me check',
    translation: 'kontrol edeyim',
    example: 'Let me check the schedule to see if the doctor is available. - Doktorun müsait olup olmadığını görmek için programı kontrol edeyim.',
    category: 'phrase',
  },
  {
    id: 'v-c-public-speaking',
    word: 'public speaking',
    translation: 'topluluk önünde konuşma',
    example: 'Many people have a deep fear of public speaking. - Birçok insanın topluluk önünde konuşma konusunda derin bir korkusu vardır.',
    category: 'phrase',
  },
  {
    id: 'v-c-what-if-she-says-no',
    word: 'what if she says no',
    translation: 'ya hayır derse',
    example: 'I want to ask her to the party, but what if she says no? - Onu partiye davet etmek istiyorum, ama ya hayır derse?',
    category: 'phrase',
  },
  {
    id: 'v-c-as-i-was-saying',
    word: 'as I was saying',
    translation: 'dediğim gibi',
    example: 'As I was saying before the phone rang, we need to finish this report. - Telefon çalmadan önce dediğim gibi, bu raporu bitirmemiz gerekiyor.',
    category: 'phrase',
  },
  {
    id: 'v-c-changing-the-subject',
    word: 'changing the subject',
    translation: 'konuyu değiştirmek',
    example: 'Anyway, changing the subject completely, what are you doing this weekend? - Her neyse, konuyu tamamen değiştirirsek, bu hafta sonu ne yapıyorsun?',
    category: 'phrase',
  },
  
  // ═══════════════════════════════════════════════════════════════
  // ACADEMIC SKILLS: High-Value Vocabulary (B1+/B2)
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'v-academic-1',
    word: 'transformational',
    translation: 'dönüştürücü',
    example: 'Technology in the hands of great teachers can be transformational. - Harika öğretmenlerin elinde teknoloji dönüştürücü olabilir.',
    category: 'adjective',
  },
  {
    id: 'v-academic-2',
    word: 'stimulate',
    translation: 'teşvik etmek, uyarmak, canlandırmak',
    example: 'Reading science fiction stimulates imagination and introduces new ideas. - Bilim kurgu okumak hayal gücünü canlandırır ve yeni fikirler sunar.',
    category: 'verb',
  },
  {
    id: 'v-academic-3',
    word: 'flexible',
    translation: 'esnek',
    example: 'Smartphones can support students and make learning more flexible. - Akıllı telefonlar öğrencileri destekleyebilir ve öğrenmeyi daha esnek hale getirebilir.',
    category: 'adjective',
  },
  {
    id: 'v-academic-4',
    word: 'anxiety',
    translation: 'endişe, kaygı',
    example: 'Yoga benefits both the body and mind by increasing flexibility and reducing anxiety. - Yoga, esnekliği artırarak ve kaygıyı azaltarak hem bedene hem de zihne fayda sağlar.',
    category: 'noun',
  },
  {
    id: 'v-academic-5',
    word: 'framework',
    translation: 'çerçeve, yapı',
    example: 'A solid essay needs a strong structural framework. - Sağlam bir makalenin güçlü bir yapısal çerçeveye ihtiyacı vardır.',
    category: 'noun',
  },
  {
    id: 'v-academic-6',
    word: 'comprehensive',
    translation: 'kapsamlı, detaylı',
    example: 'The professor provided a comprehensive overview of the historical period. - Profesör tarihi döneme dair kapsamlı bir genel bakış sundu.',
    category: 'adjective',
  },
  {
    id: 'v-academic-7',
    word: 'interactive',
    translation: 'etkileşimli',
    example: 'Interactive whiteboards allow students to participate directly in lessons. - Etkileşimli akıllı tahtalar öğrencilerin derslere doğrudan katılmasına olanak tanır.',
    category: 'adjective',
  },
  {
    id: 'v-academic-8',
    word: 'give rise to',
    translation: 'sebep olmak, yol açmak',
    example: 'The increase in car ownership has given rise to serious traffic congestion in big cities. - Araç sahipliğindeki artış, büyük şehirlerde ciddi trafik sıkışıklığına yol açtı.',
    category: 'phrase',
  },
  {
    id: 'v-academic-9',
    word: 'owing to',
    translation: 'den dolayı, yüzünden',
    example: 'Many small businesses closed owing to the economic crisis. - Birçok küçük işletme ekonomik krizden dolayı kapandı.',
    category: 'phrase',
  },
  {
    id: 'v-academic-10',
    word: 'as a consequence of',
    translation: 'sonucunda',
    example: 'Several species have become endangered as a consequence of deforestation. - Ormansızlaşmanın bir sonucu olarak birçok türün nesli tükenmekte.',
    category: 'phrase',
  },
  {
    id: 'v-academic-11',
    word: 'sustainability',
    translation: 'sürdürülebilirlik',
    example: 'Universities should require students to take at least one sustainability course. - Üniversiteler öğrencilerin en az bir sürdürülebilirlik dersi almasını zorunlu kılmalıdır.',
    category: 'noun',
  },
  {
    id: 'v-academic-12',
    word: 'procrastination',
    translation: 'erteleme, geciktirme',
    example: 'What are the main causes of procrastination among university students? - Üniversite öğrencileri arasındaki ertelemenin temel nedenleri nelerdir?',
    category: 'noun',
  },
  {
    id: 'v-academic-13',
    word: 'congestion',
    translation: 'sıkışıklık, tıkanıklık',
    example: 'Traffic congestion is a major problem in modern cities. - Trafik sıkışıklığı modern şehirlerde büyük bir sorundur.',
    category: 'noun',
  },
  {
    id: 'v-academic-14',
    word: 'specialist',
    translation: 'uzman',
    example: 'We hired a specialist to design and optimize our new e-commerce website. - Yeni e-ticaret web sitemizi tasarlaması ve optimize etmesi için bir uzman kiraladık.',
    category: 'noun',
  },
  {
    id: 'v-academic-15',
    word: 'tailor',
    translation: 'terzi / özel olarak hazırlamak, uyarlamak',
    example: 'The tailor shortened his trousers and adjusted the jacket to fit him perfectly. - Terzi, pantolonunu kısalttı ve ceketi ona tam oturacak şekilde ayarladı.',
    category: 'noun',
  },
  {
    id: 'v-academic-16',
    word: 'plumber',
    translation: 'tesisatçı',
    example: 'We had to call a plumber last week because our kitchen sink was leaking. - Geçen hafta mutfak lavabomuz sızdırdığı için bir tesisatçı çağırmak zorunda kaldık.',
    category: 'noun',
  },
  {
    id: 'v-academic-17',
    word: 'gardener',
    translation: 'bahçıvan',
    example: 'The gardener cuts the lawn and prunes the roses every Saturday morning. - Bahçıvan her cumartesi sabahı çimleri keser ve gülleri budar.',
    category: 'noun',
  },
  {
    id: 'v-academic-18',
    word: 'anecdote',
    translation: 'anektod, kısa hikaye',
    example: "He started his presentation with a funny personal anecdote to grab the audience's attention. - Dinleyicilerin dikkatini çekmek için sunumuna komik bir kişisel anekdotla başladı.",
    category: 'noun',
  },
  {
    id: 'v-academic-19',
    word: 'thesis statement',
    translation: 'tez cümlesi',
    example: 'A strong thesis statement must clearly express your main opinion at the end of the introduction. - Güçlü bir tez cümlesi, giriş paragrafının sonunda ana fikrinizi net bir şekilde ifade etmelidir.',
    category: 'noun',
  },
  {
    id: 'v-academic-20',
    word: 'paraphrase',
    translation: 'başka sözcüklerle açıklamak, açımlamak',
    example: "In academic writing, you should paraphrase the author's ideas instead of copying them directly. - Akademik yazımda, yazarın fikirlerini doğrudan kopyalamak yerine başka sözcüklerle açıklamalısınız.",
    category: 'verb',
  },
  {
    id: 'v-academic-21',
    word: 'exemplify',
    translation: 'örneklendirmek, örneklemek',
    example: 'The sample opinion essay helps to exemplify the correct structure of a body paragraph. - Örnek fikir denemesi, bir gelişme paragrafının doğru yapısını örneklendirmeye yardımcı olur.',
    category: 'verb',
  },
  {
    id: 'v-academic-22',
    word: 'topic sentence',
    translation: 'giriş cümlesi, ana fikir cümlesi',
    example: "Each body paragraph must begin with a topic sentence that presents the paragraph's main idea. - Her gelişme paragrafı, paragrafın ana fikrini sunan bir giriş cümlesi ile başlamalıdır.",
    category: 'noun',
  },
  {
    id: 'v-academic-23',
    word: 'concluding sentence',
    translation: 'sonuç cümlesi',
    example: "A concluding sentence wraps up the paragraph's arguments and links back to the main point. - Bir sonuç cümlesi, paragrafın argümanlarını özetler ve ana fikre geri bağlar.",
    category: 'noun',
  },
  {
    id: 'v-academic-24',
    word: 'to sum up',
    translation: 'özetlemek gerekirse, özetle',
    example: 'To sum up, public transportation reduces traffic congestion and helps the environment. - Özetlemek gerekirse, toplu taşıma trafik sıkışıklığını azaltır ve çevreye yardımcı olur.',
    category: 'phrase',
  },
  {
    id: 'v-academic-25',
    word: 'revision',
    translation: 'revizyon, gözden geçirme, tekrar',
    example: 'Careful revision is essential before submitting your final essay to your teacher. - Final denemenizi öğretmeninize teslim etmeden önce dikkatli bir şekilde gözden geçirme (revizyon) şarttır.',
    category: 'noun',
  },
  {
    id: 'v-academic-chunk2-01',
    word: 'let go of',
    translation: 'Serbest bırakmak, vazgeçmek, geride bırakmak',
    example: 'Sometimes you need to let go of old habits to make progress in your life. - Hayatında ilerleme kaydetmek için bazen eski alışkanlıklarından vazgeçmen gerekir.',
    category: 'phrase',
  },
  {
    id: 'v-academic-chunk2-02',
    word: 'mess with',
    translation: 'Uğraşmak, karışmak, kurcalamak, bulaşmak',
    example: 'Please do not mess with my computer settings while I am working on my project. - Projem üzerinde çalışırken lütfen bilgisayarımın ayarlarını kurcalamayın.',
    category: 'phrase',
  },
  {
    id: 'v-academic-chunk2-03',
    word: 'honour',
    translation: 'Onurlandırmak, saygı göstermek, sözünü tutmak',
    example: 'It is important to honour your commitments and meet your project deadlines on time. - Taahhütlerinize saygı göstermek ve proje teslim tarihlerinizi zamanında karşılamak önemlidir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk2-04',
    word: 'adopt',
    translation: 'Benimsemek, kabul etmek, uygulamaya koymak',
    example: 'Many schools decided to adopt new teaching methods to improve student engagement. - Birçok okul, öğrenci katılımını artırmak için yeni öğretim yöntemlerini benimsemeye karar verdi.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk2-05',
    word: 'merit',
    translation: 'Değer, erdem, hak etmek',
    example: 'The selection committee will judge each university application solely on its academic merit. - Seçici kurul, her üniversite başvurusunu yalnızca akademik değerine göre değerlendirecektir.',
    category: 'noun',
  },
  {
    id: 'v-academic-chunk2-06',
    word: 'procrastination',
    translation: 'Erteleme, erteleme alışkanlığı',
    example: 'Procrastination is a major obstacle that prevents many students from achieving success. - Erteleme alışkanlığı, birçok öğrencinin başarıya ulaşmasını engelleyen büyük bir engeldir.',
    category: 'noun',
  },
  {
    id: 'v-academic-chunk2-07',
    word: 'arise',
    translation: 'Ortaya çıkmak, baş göstermek',
    example: 'Several unexpected problems may arise during the implementation of the new policy. - Yeni politikanın uygulanması sırasında beklenmedik birkaç sorun ortaya çıkabilir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk2-08',
    word: 'lead to',
    translation: 'Yol açmak, sebep olmak',
    example: 'Poor time management skills can easily lead to high levels of stress and lower grades. - Zayıf zaman yönetimi becerileri, kolayca yüksek düzeyde strese ve düşük notlara yol açabilir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk2-09',
    word: 'owing to',
    translation: '-den dolayı, yüzünden, sebebiyle',
    example: 'The outdoor graduation ceremony was cancelled owing to heavy rain. - Mezuniyet töreni, şiddetli yağmur nedeniyle iptal edildi.',
    category: 'preposition',
  },
  {
    id: 'v-academic-chunk2-10',
    word: 'consequence',
    translation: 'Sonuç, akıbet',
    example: 'Rising sea levels are a direct consequence of global climate change. - Yükselen deniz seviyeleri, küresel iklim değişikliğinin doğrudan bir sonucudur.',
    category: 'noun',
  },
  {
    id: 'v-academic-chunk2-11',
    word: 'influence',
    translation: 'Etkilemek, tesir etmek, etki',
    example: "Peer pressure can heavily influence a teenager's choices and behavior. - Akran baskısı, bir gencin seçimlerini ve davranışlarını büyük ölçüde etkileyebilir.",
    category: 'verb',
  },
  {
    id: 'v-academic-chunk2-12',
    word: 'affect',
    translation: 'Etkilemek, tesir etmek',
    example: 'Extreme weather conditions can severely affect agricultural production. - Aşırı hava koşulları tarımsal üretimi ciddi şekilde etkileyebilir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk3-01',
    word: 'obstacle',
    translation: 'Engel, mani',
    example: 'Language barriers can become a significant obstacle when you are trying to adapt to a new country. - Yeni bir ülkeye uyum sağlamaya çalışırken dil engelleri önemli bir engel haline gelebilir.',
    category: 'noun',
  },
  {
    id: 'v-academic-chunk3-02',
    word: 'barrier',
    translation: 'Bariyer, sınır, engel',
    example: 'A severe language barrier often leads to emotional isolation and stress for international students. - Ciddi bir dil engeli, uluslararası öğrenciler için genellikle duygusal yalıtıma ve strese yol açar.',
    category: 'noun',
  },
  {
    id: 'v-academic-chunk3-03',
    word: 'major concern',
    translation: 'Büyük endişe, başlıca kaygı',
    example: 'One major concern regarding fully AI-driven classrooms in 2040 is the lack of face-to-face social-emotional development. - 2040 yılındaki tamamen yapay zeka odaklı sınıflarla ilgili büyük bir endişe, yüz yüze sosyal-duygusal gelişim eksikliğidir.',
    category: 'phrase',
  },
  {
    id: 'v-academic-chunk3-04',
    word: 'lead to',
    translation: 'Yol açmak, sebep olmak',
    example: 'Excessive smartphone usage can lead to severe communication barriers and social isolation among youth. - Aşırı akıllı telefon kullanımı, gençler arasında ciddi iletişim engellerine ve sosyal yalıtıma yol açabilir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk3-05',
    word: 'result in',
    translation: 'Sonuçlanmak, -e yol açmak',
    example: 'Misinterpreting local eye contact and hand gestures can result in immediate cultural misunderstandings. - Yerel göz temasını ve el hareketlerini yanlış yorumlamak, anında kültürel anlaşmazlıklarla sonuçlanabilir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk3-06',
    word: 'overcome',
    translation: 'Üstesinden gelmek, aşmak',
    example: 'With proper pre-trip research and an open-minded attitude, travelers can successfully overcome culture shock. - Seyahat öncesi doğru araştırma ve açık görüşlü bir tutumla, gezginler kültür şokunun üstesinden başarıyla gelebilir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk3-07',
    word: 'tackle',
    translation: 'Ele almak, çözmeye çalışmak, başa çıkmak',
    example: 'To tackle the issue of digital distraction, schools should encourage periodic digital detoxes and group activities. - Dijital dikkat dağınıklığı sorununu ele almak için okullar periyodik dijital detoksları ve grup aktivitelerini teşvik etmelidir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk3-08',
    word: 'resolve',
    translation: 'Çözmek, karara bağlamak',
    example: 'Polite clarifying questions can resolve communication style clashes before they escalate into conflicts. - Kibar netleştirme soruları, iletişim tarzı çatışmalarını çatışmaya dönüşmeden önce çözebilir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk3-09',
    word: 'can be solved',
    translation: 'Çözülebilir, çözülmesi mümkündür',
    example: 'The problem of urban traffic congestion can be solved through massive investments in efficient public transport networks. - Şehir içi trafik sıkışıklığı sorunu, verimli toplu taşıma ağlarına yapılan büyük yatırımlarla çözülebilir.',
    category: 'phrase',
  },
  {
    id: 'v-academic-chunk3-10',
    word: 'can be reduced',
    translation: 'Azaltılabilir',
    example: 'Culture shock can be reduced if travelers watch informative local videos and read simple cultural guides beforehand. - Gezginler önceden bilgilendirici yerel videolar izler ve basit kültürel kılavuzlar okursa kültür şoku azaltılabilir.',
    category: 'phrase',
  },
  {
    id: 'v-academic-chunk3-11',
    word: 'can be prevented',
    translation: 'Önlenebilir, engel olunabilir',
    example: 'Unhealthy eating habits among university students can be prevented by offering affordable healthy options on campus. - Üniversite öğrencileri arasındaki sağlıksız beslenme alışkanlıkları, kampüste uygun fiyatlı sağlıklı seçenekler sunularak önlenebilir.',
    category: 'phrase',
  },
  {
    id: 'v-academic-chunk4-01',
    word: 'solve',
    translation: 'çözmek, çözüm bulmak',
    example: 'Educators are looking for creative ways to solve the problem of student distraction in online learning. - Eğitimciler, çevrimiçi öğrenmede öğrencilerin dikkat dağınıklığı sorununu çözmek için yaratıcı yollar arıyorlar.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-02',
    word: 'overcome',
    translation: 'üstesinden gelmek, aşmak',
    example: 'To overcome language barriers, international students are encouraged to participate in local speaking clubs. - Dil engellerini aşmak için uluslararası öğrencilerin yerel konuşma kulüplerine katılmaları teşvik edilmektedir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-03',
    word: 'tackle',
    translation: 'ele almak, üzerine gitmek, çözmeye çalışmak',
    example: 'Governments must implement strict regulations to tackle the rise of urban air and noise pollution. - Hükümetler, kentsel hava ve gürültü kirliliğindeki artışı ele almak için katı düzenlemeler uygulamalıdır.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-04',
    word: 'address',
    translation: 'hitap etmek / ele almak, yönelmek, çözmeye çalışmak',
    example: 'The university administration organized workshops to address the issue of academic anxiety among freshmen. - Üniversite yönetimi, birinci sınıf öğrencileri arasındaki akademik kaygı sorununu ele almak için atölyeler düzenledi.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-05',
    word: 'deal with',
    translation: 'ile başa çıkmak, ele almak, uğraşmak',
    example: 'Many travelers find it difficult to deal with the initial emotional stress of culture shock. - Birçok gezgin, kültür şokunun ilk duygusal stresiyle başa çıkmakta zorlanır.',
    category: 'phrase',
  },
  {
    id: 'v-academic-chunk4-06',
    word: 'cope with',
    translation: 'ile baş etmek, üstesinden gelmek, göğüs germek',
    example: 'Developing effective time management skills is essential to cope with the heavy academic workload at university. - Üniversitedeki yoğun akademik iş yüküyle baş edebilmek için etkili zaman yönetimi becerileri geliştirmek şarttır.',
    category: 'phrase',
  },
  {
    id: 'v-academic-chunk4-07',
    word: 'handle',
    translation: 'başa çıkmak, yönetmek, ele almak',
    example: 'The local municipality plans to handle the increasing volume of traffic by promoting public transport. - Yerel belediye, toplu taşımayı teşvik ederek artan trafik hacmiyle başa çıkmayı planlıyor.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-08',
    word: 'reduce',
    translation: 'azaltmak, düşürmek',
    example: 'Improving energy efficiency in residential buildings is a key strategy to reduce global carbon emissions. - Konut binalarında enerji verimliliğinin artırılması, küresel karbon emisyonlarını azaltmak için temel bir stratejidir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-09',
    word: 'improve',
    translation: 'geliştirmek, iyileştirmek',
    example: 'Modernizing outdated library spaces can significantly improve student attendance and community engagement. - Eski kütüphane alanlarının modernize edilmesi, öğrenci katılımını ve topluluk bağını önemli ölçüde artırabilir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-10',
    word: 'prevent',
    translation: 'önlemek, engel olmak',
    example: 'Setting strict daily screen time limits is an effective way to prevent digital media addiction among youth. - Günlük ekran süresine katı sınırlar koymak, gençler arasında dijital medya bağımlılığını önlemenin etkili bir yoludur.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-11',
    word: 'steady',
    translation: 'istikrarlı, düzenli, sabit',
    example: 'The graph illustrates a steady increase in global temperatures over the past three decades. - Grafik, son otuz yılda küresel sıcaklıklarda istikrarlı bir artış olduğunu göstermektedir.',
    category: 'adjective',
  },
  {
    id: 'v-academic-chunk4-12',
    word: 'represent',
    translation: 'temsil etmek, göstermek',
    example: 'Each bar on the chart represents the average level of noise pollution in a specific urban neighborhood. - Grafikteki her çubuk, belirli bir kentsel mahalledeki ortalama gürültü kirliliği seviyesini temsil eder.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-13',
    word: 'illustrate',
    translation: 'görselle açıklamak, resmetmek, göstermek',
    example: 'These statistics clearly illustrate the correlation between excessive screen time and sleep deprivation. - Bu istatistikler, aşırı ekran süresi ile uykusuzluk arasındaki ilişkiyi açıkça göstermektedir.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-14',
    word: 'contrast',
    translation: 'karşılaştırmak, tezat oluşturmak',
    example: 'When describing visuals, it is helpful to contrast the current findings with the data we saw before. - Görselleri tasvir ederken, mevcut bulguları daha önce gördüğümüz verilerle karşılaştırmak faydalıdır.',
    category: 'verb',
  },
  {
    id: 'v-academic-chunk4-15',
    word: 'summarize',
    translation: 'özetlemek',
    example: 'The final slide of the presentation will summarize the main recommendations for tackling urban traffic jams. - Sunumun son slaydı, kentsel trafik sıkışıklığıyla başa çıkmak için temel önerileri özetleyecektir.',
    category: 'verb',
  },
];
